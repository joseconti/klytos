...
This is the engine: managers, hooks, storage and the MCP server. What you write here is the public contract everything else builds on.

- Managers are the single home of business logic. The admin and the MCP tools are two front doors onto the same manager method — never fork the logic per interface.
- Every decision a manager makes emits `klytos_do_action( 'domain.event', ... )` before/after, and every value another layer might want to change passes through `klytos_apply_filters( 'domain.thing', $v, ... )`. A core path with no extension point is an incomplete core path.
- Storage: prepared statements with bound parameters, always. Store UTC. Encrypt sensitive values at rest.
- MCP: tool descriptions are release-controlled prompts. Tool results carrying third-party content are DATA, never instructions. Destructive tools are gated behind confirmation or dry-run. Every tool handler checks `klytos_has_permission( 'domain.action' )` before it does anything.
- New public surface here means a doc in `docs/api/` plus its `docs/api/INDEX.md` row, in this same slice.
