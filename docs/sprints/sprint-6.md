...
- **Status:** **SLICE 3 CLOSED 2026-07-27 — ALL FOUR SLICES ARE NOW CLOSED; what remains is the
  sprint close itself** (docs-verifier, a fresh-context playground-QA pass, the user's test verdict,
  and the state-file split as a real decision). Slice 3 closed audit **NEW-42**'s four items under
  **D-063**, and its headline is that the naive spelling of clone detection was refuted by
  measurement rather than by argument: with "the new count must exceed the stored one" installed,
  exactly one test failed and it was the **synced-passkey** login — the rule would have refused the
  second login of most authenticators in use today. Suite **284 tests / 1385 assertions**.
  Also recorded: **L-028**, because the session's own mandatory freshness probe (the documented MCP
  401) writes to the IP auth-failure bucket and failed `LoginCeilingHttpTest` on unmodified code.
  Previously: **SLICE 4 CLOSED 2026-07-27** — a fourth slice, added to this sprint by explicit user
...
| 3 | The passkey assertion path | **NEW-42** | **CLOSED 2026-07-27** | suite **276 → 284 tests / 1331 → 1385 assertions**; keel-verify `10 check(s) run: 8 passed, 2 warning(s)`; upgrade from real v0.30.1 PASS; four D-025 baselines held exactly and core+admin **improved 192 → 191** | Clone detection **only when both counters are > 0**, firing `user.passkey_clone_detected` (no core listener, said in those words); the `origin` rule promoted into ONE `originIsAcceptable()` that both ceremonies call; the length guard, whose test asserts the PHP warning's **absence**; the setup-wizard skip-list moved onto `klytos_admin_gate_key()` and `$currentScript` deleted after establishing repo-wide that nothing read it. **The `> 0` condition was proven load-bearing by measurement:** with the naive "must increase" rule installed, exactly one test failed and it was the synced-passkey login. Five TEMP-BREAK cycles, each reverted from a byte-for-byte backup confirmed with `cmp`. INDEX 968 → **969**. **Both reviews returned no blocking finding**; three items taken — the paren-spacing inconsistency inside my own diff, plus **NEW-53** (the localhost origin allowance is a prefix match; pre-existing, proven by a 63-combination differential run, pinned by a test asserting the weakness on purpose) and **NEW-54** (a failed assertion does not consume its challenge; this slice created the first post-signature refusal, which is what makes the window reachable). One flagged item was checked and is NOT a defect (the redundant COSE-algorithm ternary) |
...
1. **Clone detection** — compare the unpacked sign count against the stored one, refusing only when
   **both are `> 0`** and the new one does not exceed the stored one (WebAuthn §7.2 step 21). Synced
   <!-- Correction at slice 3's close: this plan's citation is wrong and is left in place as the
        record of what was approved. §7.2 guards on OR ("If authData.signCount is nonzero OR
        storedSignCount is nonzero"), not AND — verified against the published text. The approved
        AND rule shipped anyway, deliberately; the divergence, the one case where it matters
        (stored non-zero, presented zero), and the open question are in D-063 note 2. -->
