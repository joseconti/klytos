...
- **Status:** **CLOSED 2026-07-20** — all 10 slices closed
...
| 9 | `scripts/keel-verify` + regenerable INDEX | Phase 5 §1a, D-04 | **closed 2026-07-20** | **PASS** — **142 tests/617 assertions** green; `keel-verify` 2 to **9 checks**, full output pasted in `docs/05-test-points.md`; **every new check proven to FAIL on an injected violation** (10 probes, all fired, tree left clean) and both WARNs proven to go quiet when fixed; upgrade from real v0.30.1 passing; all four D-025 baselines held plus `scripts` recorded fresh at 0/2 ; commit `e8e3938` | **Settled with the user: a VERIFIER, not a regenerator** — INDEX's Summary counts are checked against its own rows (green at birth: 145/97/306/116/206/34/26/19 = 949) rather than a bespoke extractor rewriting a correct file on a silent mis-extraction. New **WARN** tier reports a broken property whose fix another phase owns, without failing the build — the treatment sprint-1.md already scoped for **H-01**. **Two release-packaging defects found, both from defining *distributable* as `git check-attr` rather than a hand-written list: NEW-27** — all 16 in-product guides are stripped from every release archive, so `klytos_list_guides` and `klytos_get_guide` return nothing on a real install although the tools' own descriptions declare several REQUIRED before creating content (verified by extracting the archive; the directory ships EMPTY, and v0.30.1 has no assets so the updater falls back to the zipball, which honours export-ignore); **NEW-28** — `scripts/` ships to the web root and `router.php` and `upgrade-assert.php` **executed over HTTP**, the former returning an internal disclosure page. SAPI guards fixed in path (D-031); packaging left to Phase 7. **CI workflow written** and it **seeds the playground**, because 91 of 138 tests skip without one — an un-seeded CI would report green having run 34% of the suite; a skip is promoted to a hard failure. 8.2/8.3 run the suite, 8.1 gets syntax-only (D-027 stated, not papered over). All four **permission allow-lists** written; **`docs/flows/`** created with 3 targeted journeys; `scripts/` added to the phpcs scan set — it had **never been linted**, the slice-7 shape again. Decision **D-045**; lesson **L-016** — **three measurements produced a false PASS in this session**, including a guard test that passed against the guard not firing |
...
| Requirement | Status |
|---|---|
| Full suite green (every test, not only this sprint's) | **yes** — `OK (142 tests, 617 assertions)` |
| `keel-verify` output pasted | **yes** — full output in `docs/05-test-points.md`, slice-9 evidence block; 9 checks, 2 warnings, exit 0 |
| Upgrade tested from the REAL previous version | **yes** — `UPGRADE TEST PASSED (v0.30.1 -> 0.31.1-beta.1)` |
| Lint baselines held | **yes** — core+admin 193/488, plugins 113/109, tests 0/0, public 0/0, **scripts 0/2 (new)** |
| `code-reviewer` + `security-auditor` per slice | **yes** — every slice 3-9, on a finished diff (L-015) |
...
The authorization axis is closed and **provable**: S-01 through S-09, S-11, S-12, the CSP fail-open,
T-01, T-02, H-04, NEW-01, NEW-14 and NEW-16 are all CLOSED, each with a named test demonstrated to
FAIL against the unfixed code before it was trusted. Five single points now exist where there were
none or many — one authorization decision (`UserManager::hasPermission()`), one admin gate
(`admin/bootstrap.php`), one safe fetcher (`SafeHttp`), one rate limiter (`MCP\RateLimiter`), one
header policy (`Auth::buildSecurityHeaders()`) — and `scripts/keel-verify` mechanically defends nine
promises instead of two.

### Stated plainly, because closing on a high note alone would be dishonest

- **The product's primary interface is still ungated.** NEW-02: 172 MCP tools, zero permission
  checks. Sprint 1 gated the admin panel; Sprint 2 (D-020) gates MCP. Until it runs, any
  application-password holder has owner-equivalent power over the CMS.
- **Only one account can actually log in** (NEW-11). Sprint 1 gated a multi-role system that, in
  production, only `config['admin_user']` can enter.
- **Five known-vulnerable vendored dependencies remain** (NEW-05 / D-029).
- **A released install has no in-product guides at all** (NEW-27) — found in the last slice, and it
  means the AI-first CMS ships without the instructions its own tools tell the AI to read first.

### Risks, final state

1. Default-deny lockout — **materialised as designed and mitigated** (260-request role walk, slice 4).
2. Slice 2 unbounded — **resolved**: 5 medium CVEs, bounded, deferred by D-029.
3. Playground writes real local credentials — held; gitignored, the pre-commit gate is the net.
4. Integration tier storage isolation — **resolved** by D-030, extended to hook state by D-042.
5. Never run the web installer in-tree — held; documented loudly in `docs/playground.md`.
