...
| **42 Template preview** (added by **D-104**, 2026-08-10) | A preview endpoint, then a DOM check runner, then a default-template store — three product surfaces, in that order | **The emptiest screen the per-screen survey has measured.** None of §42's nine checks exists anywhere in the product: the only adjacent primitive is `ThemeManager::contrastPairs()` (`theme-manager.php:247-286`), which scores the THEME's colours and never sees a rendered template. There is no default-template concept at all to "set as default" — no `default_template` or `set_default` key exists under `installer/`, per post type or globally — so §42's own hard-check delta has nothing to write and nothing to prevent. And there is no preview endpoint: both preview surfaces render `srcdoc` with the bare wrapper fragment `renderPage()` returns (`page-template-manager.php:87`, `:384-387`), with no `<!DOCTYPE>`, no `<html lang>`, no `<head>` and no site stylesheet — so three of the nine checks would fail structurally for EVERY template whatever its quality, and contrast is untestable because the theme CSS is not in the frame. The screen is also unreachable from the UI: nothing links to `template-preview.php` (`templates.php:117` links to its own `?preview=`) |
...
| **37 x402 settings** | **Exempt agents** (repeatable rows) | Not merely unbacked — **inverted**. The product's repeatable list is `custom_bot_user_agents`, which ADDS agents to the charged set; there is no subtractive list, and `known_bot_user_agents` is a class constant with no removal affordance. An agent not on the list already passes free, so an *exempt* list only means anything as an override of the built-ins: new gate semantics with their own `HtaccessWriter` consequences, not a card (D-098) |
| **37 x402 settings** | the **enable/disable switch** delta | There is no global on/off. `x402_enabled` is per PAGE and `x402_default_enabled` per POST TYPE, each on its own screen, and the `.htaccess` rules are written unconditionally at `build.after`. The prototype's three switch-shaped controls all sit on the unbacked Who pays card and none maps to a stored setting, so the delta is deferred WITH that card and every control on the screen is checkbox + Save — §37's own second sentence. Reversible in one block if Design says otherwise (D-098) |
...
| **26 Privacy** | **Per-section method and status** (the list-table card, with its `Last run` column and its Tasks delta) | Deferred for **four independent reasons, none of which is DR-006's missing widths** — that block was real and was never the binding one. (1) There is no site-wide section registry: `collectErasableData()` takes a `$userId` and the `privacy.erasable_data` filter is handed a user, so the product cannot enumerate sections without naming a person. (2) **`Last run` is stored nowhere** — no collection holds a per-section erasure timestamp. (3) The status vocabulary `Automatic` / `Manual` / `Not covered` maps to nothing: the product's axes are `erasure_method` ∈ {delete, anonymize} and an `erasable` flag with a `retention_reason`. (4) "A section that is Not covered is a task, and it appears on Tasks" needs a generator, and tasks are explicitly created records (`TaskManager::create()`) with nothing deriving one from privacy coverage. Each is a product surface (D-099) |
| **27 Profile** | **Sessions** (list-table, "including MCP clients") | **The second card on this list that was recorded as DR-006-blocked and never was** — entry 26's correction, arriving again. There is no session registry of any kind: sessions are plain `$_SESSION` (`core/auth.php`), so no device, client, IP, location or start time is stored anywhere and there is nothing for a table to draw. The only revocation primitive is `UserManager::forceLogoutAllSessions()` (`user-manager.php:595`), which is all-or-nothing and cannot revoke the single row a Revoke action would name. And the MCP clients are **not per person**: `core/mcp/oauth-server.php:80-87` stores `client_id`, `name`, `redirect_uri` and no user id at all, so they are install-wide and already live on entry 8 behind `mcp.manage` — an editor's own profile cannot list them, and a second surface for them is the duplication D-090 refused. A per-session registry is a storage surface with its own authorization review, not a card (D-100) |
| **27 Profile** | **Preferences** — dock mode · sidebar collapse · table density · last filter | Four of the delta's five preferences. The theme IS built, because `admin/api/theme.php` persists it. The other four are not deferred for scheduling: **dock mode** and **sidebar collapse** exist only as `localStorage` keys the server never sees (`klytos_nav_expanded`, `klytos_sidebar_collapsed`), so a control for them would be JavaScript-only on a screen where every other control works without it — and the delta itself says each one "takes effect immediately", which the product cannot honour for a value it does not hold. **Table density** and **last filter** exist nowhere in the tree. Per-user preference storage would be new product: `MetaManager` could hold them, but each one also needs the screen that obeys it (D-100) |
| **28 Licence** | **Activated domains** (list-table) | **The FOURTH card recorded as DR-006-blocked that the widths were never blocking** — after entry 26 and entry 27 twice. `License::activate()` writes exactly ONE `domain` and one `site_url` (`core/license.php:75-87`); the licence API is never asked for a domain list and `License` cannot enumerate one. There is no collection for a table to draw, at any column width. A per-domain registry is a storage surface owned by the licence SERVER, not by this install (D-101) |
| **28 Licence** | **Entitlements** (stat row) | `plan` is a bare string copied out of the activation response, and the product stores no entitlement, quota or feature record of any kind. Nothing anywhere reads `plan` to grant or refuse anything — `License::isActive()` itself has no caller in the tree. A stat row here would be numbers nobody measured (D-101) |
| **32 Taxonomies** | the terms **list-table**, and specifically its **`Count`** column | **The FIFTH card recorded as DR-006-blocked that the widths were never the whole blocker** — after entry 26, entry 27 twice and entry 28. **No record anywhere in the product is ever associated with a term.** `PageManager` stores no term; no MCP tool assigns one (`page-tools.php` only tells the assistant which taxonomies exist); the only count in the tree is entry 19's count of terms IN a taxonomy, which is a different fact from the number of records carrying a term. So the specified five-column table cannot be built as specified even with DR-006's widths in hand — one of its columns would be a number nobody measured. Term↔record association is a storage surface with its own MCP and build consequences, not a column. The add-term FORM half is built (D-102) |
| **18 x402 dashboard** | **Settlement lag** (one of the five stat cards) | **The product records ONE instant per transaction.** `TransactionLog::log()` writes `created_at` and nothing else time-shaped — `facilitator_ok` is a bool, `tx_hash` a frequently-empty string. A lag is a distance between two instants and there is one. Measuring it needs the payment gate to record a settlement timestamp, a migration for records already stored on released installs, and a defined meaning for a payment that never settles (D-112, DR-014) |
| **13 Tasks** | **Due this week** · **Overdue** (two of the four stat cards) | **A Klytos task has no due date, anywhere.** `TaskManager::create()` writes ten fields and none is one; `update()`'s allow-list adds none; storage holds none; neither the MCP tools nor the front-end review widget — the two ways a task is really created — send one. Both cards would be numbers nobody measured, and §13's "Overdue is never red alone" delta protects a state that cannot occur. Due dates are a capability with their own field, writer, validator, MCP surface, sort order and timezone question — not a card (D-111, DR-013) |
| **13 Tasks** | the **source** line ("raised by System integrity") | `created_by` is a **user id**; nothing in the product records a SUBSYSTEM as the origin of a task. The row shows the page the task is on and when it was raised, both real (D-111, DR-013) |
| **24 Webhooks** | **HMAC secret** (read-only mono + rotate) | The product has no rotate of any kind: `WebhookManager::update()`'s `$updatable` list is `['url','events','description','status']` and `secret` is excluded, so nothing in the tree can regenerate one. The card also assumes ONE site-wide secret while the product stores one PER WEBHOOK. The manifest's own two-step confirm names the consequence — existing endpoints stop being able to verify deliveries — which is a product change on live integrations, not a card. The secret IS handed over once, on the request that creates its webhook (D-103) |
| **24 Webhooks** | **Delivery log** (list-table) | **The SIXTH card recorded as DR-006-blocked whose real obstruction was never the widths** — after entry 26, entry 27 twice, entry 28 and entry 32. `logDelivery()` writes exactly five fields (`webhook_id`, `success`, `attempts`, `error`, `timestamp`), so of the six specified columns **Event**, **Code** and **Duration** have no data source at all: no event name is recorded, the HTTP code is not kept (only a bool and a free-text error that sometimes reads "HTTP 502"), and no duration is measured anywhere. The delta "Retry is a form post per delivery" has no primitive either — the log does not keep the payload, so there is nothing to re-send. Three columns of six would be numbers nobody measured (D-103) |
...
### The engine interiors — stage 6, D-104 (2026-08-10)

Everything above is a CARD the delivery draws over data the product does not hold.
These three are different in kind and are recorded apart so the distinction is not
lost: the delivery specifies the **interior of software this product did not
write**. The chrome around each one is built to the letter; the interior is not.

| Entry | Interior | Why it is not Phase 4 work |
|---|---|---|
| **2 Page editor** | The canvas: blocks as bordered cards with a mono eyebrow, each a `role="group"` named "*Kind* block — *first words*", each with a `contenteditable role="textbox" aria-multiline`, plus the **"Edit as form"** fallback page per block, and the **hard publish blockers** (no `<h1>`; an image with no alt decision) | The product mounts **Gutenberg** (`isolated-block-editor.js`) or **TinyMCE**, chosen per post type by `PostTypeManager`'s stored `editor` and branched at `page-editor.php:657`; the vendored files say "NEVER modify these" (`:707`, `:713`). The block cards, their roles, their names and their focus model are Gutenberg's own DOM, not markup this build emits. "Edit as form" is a whole new `record-form` surface **per block kind** and exists nowhere. The publish blockers have no validator of any kind: nothing in `installer/core/` inspects content for an `<h1>` or an alt decision. Autosave, by contrast, **does** exist (`admin/api/autosave.php`, `klytos-editor.js`, 60s) and is built |
| **23 Terminal** | The stream: a `<pre>` whose LINES are focusable and selectable with `↑`/`↓` and `Enter`, plus streamed output, elapsed seconds, an exit code with its meaning, a working Stop/`Ctrl+C`, and the 5,000-line truncation notice | The product renders into an **xterm.js canvas** (`terminal.php:241`, `:301-337`), which has no line-focus model — and `accessibility.md` §7.1 makes that keyboard operability a **CONDITION** of the one sub-24px target the admin allows, so the exception cannot be claimed here. Behind it: `dispatch()` buffers with `ob_start()`/`ob_get_clean()` and returns one string (`terminal-executor.php:146-156`), nothing measures duration anywhere in the path, the declared return type is `array{success: bool, output: string}` with no numeric status (`:122`), and handlers run synchronously in-process with no interrupt point, so there is nothing for a Stop to cancel |
| **12 AI chat** | The streaming turn (`role="log"`, aria-hidden node swapped on completion, "Jump to latest"), the **Stop** that keeps a partial turn, tool calls in **running** and **needs permission** states, "Load earlier messages", the starters drawn from the last screen visited, the context chip row, and the whole **copilot dock** (docked / collapsed / floating / full, remembered per user) | `api/ai-chat.php:171-191` responds **once** with the whole result and `chat-engine.php` has no streaming path, so a partial turn cannot exist — and Stop, a running tool call and an inline permission confirm are all states of a partial turn. Consent today is a sentence in the system prompt (`chat-engine.php:401`), not a UI round trip. `getChat()` reads every message with no limit/offset (`chat-manager.php:108-116`), so "Load earlier messages" has no query to make. Nothing in the tree records a last screen visited. The dock is an empty landmark and nothing else (`templates/footer.php:40`): no mode enum, no per-user store, no launcher |

Review trigger, for every row in §0c — the cards above and the three interiors:
**the close of the Phase 4 build**, exactly as §0b.
