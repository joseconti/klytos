...
| D21 | **Replay of a single-use magic login link** | `TwoFactor::markMagicLinkUsed()` writes `used = true` against the record's STORAGE id and `verifyMagicLink()` refuses any link already marked. Expired and used links are purged by `cleanupMagicLinks()` | `IN PLACE` | **Was NOT in place until 2026-08-29.** The write sat behind `if ( $id )` reading a field the writer never stored, so it never ran and a link verified repeatedly for its whole 10-minute lifetime. `MagicLinkSingleUseTest` was seen failing on the replay before the fix (D-115) |
| D22 | **A GDPR erasure that reports success without erasing** | `PrivacyManager::anonymizeAuditLog()` writes the anonymised entry back to the record's own STORAGE id, so the PII-bearing original is overwritten rather than duplicated | `IN PLACE` | **Was NOT in place until 2026-08-29.** The id was rebuilt as `Ymd-His-0000` where real ids end in `randomHex(4)`, so the write created an orphan and left the original intact. `GdprAuditAnonymisationTest` proven red on a planted defect in the integration tier (D-115) |
| D23 | **Personal data kept past its stated retention** | `AnalyticsManager::prune()` and `AuditLog::prune()` delete by STORAGE id; the daily `klytos.analytics_prune` / `audit_log_prune` crons therefore complete | `IN PLACE` | **Was NOT in place until 2026-08-29.** Both pruners deleted with an id field that is never stored: the analytics cron threw every night into `error_log` and the audit pruner silently deleted nothing. `AnalyticsRetentionTest` seen failing (D-115) |
