...
- **`SPEC/navigation.md`** — the sidebar's normative contents: the eight groups in order,
  every item's label, glyph, target and count, plugin placement, capability rule.
...
Monetisation · Compliance · System · Account**. Plugins may add one entry. The full
contents of each group — items, labels, glyphs, counts — are normative in
**`SPEC/navigation.md`**; where a prototype's `navGroups` array disagrees with it, that file
wins.
...
`SPEC/assets-index.md` §3. Sidebar glyphs are assigned in `SPEC/navigation.md`.
...
│  ├─ navigation.md                        ← the sidebar: groups, items, glyphs, counts
