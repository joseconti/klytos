...
## Resolved in DR-003 (the sidebar, and the glyphs it draws)

### 19. The sprite was 19 glyphs short of what the sidebar draws
**Resolved by counting: added.** The union of the prototypes' `navGroups` uses 35 distinct
glyphs; nineteen had no `<symbol>`. All nineteen are drawn in the existing geometry and the
sprite is now **87 symbols**. Enumerated in `SPEC/assets-index.md` §3, which also gains a
per-shell glyph list.

### 20. Three shell controls had no glyph named anywhere
**Decided in design.** The drawer button below 900px is **`ks-menu`** — the twentieth symbol
added, because every other menu-ish glyph in the sprite is already spoken for. The rail's
"Expand navigation" button reuses **`ks-chevron_right`**. The theme toggle is **text-only and
takes no glyph**: its visible text already states the target state, and an icon-only
light/dark toggle is a known "which one am I in?" trap. `SPEC/navigation.md` §8.

### 21. The sidebar's contents were never specified
**Decided in design: one normative file, `SPEC/navigation.md`.** README named the eight
groups and stopped; `template-shell.md` specified behaviour, not contents; each prototype drew
a partial `navGroups` and no file drew all eight. The new file gives the fixed group order,
every item with its label, glyph, manifest entry and count, and states that it — not a
prototype array — is the source of truth. Five nav labels differ from their `<h1>` on purpose
(Overview, Assets, Theme, MCP, Agent payments) and each is justified in place.

### 22. Blocks — Content or Design?
**Decided: Design.** Two prototypes said Content, four said Design. Entry 21 is a gallery of
registered block *types* — an inventory of the building set, like Templates and Theme. Content
holds records a person authored. `navigation.md` §3.

### 23. What is *Guides (16)*?
**Decided: dropped.** It is not a manifest entry, has no `SPEC/screens/` file, and nothing
links to it. Help is already answered by `⌘K` → `help` and by contextual help in each screen.
If a documentation library is wanted later it arrives as a manifest entry first and joins
**Site**. The manifest stays at 44. `navigation.md` §4.

### 24. Where do the seven entries with no nav home live?
**Decided: six are child screens, one is not shipped.** Page editor, Post type, Block data and
Template preview are reached only from their parent list (and while one is open the *parent's*
nav item keeps `aria-current`); Verify and the Setup wizard have no shell at all; the States
sheet is a specimen, not a screen. Table with the parent for each: `navigation.md` §5.

### 25. Plugin items, and capability-gated items
**Decided in design.** A plugin's one item lands in the group its primary capability implies
(mapping table in `navigation.md` §6), never in *Site* or *Account*, always after the core
items, alphabetically among plugins, and never with a count. Past five plugin items in one
group the group shows five plus **More plugins** → `plugins.php`, which is always the complete
list. An item the person lacks the capability for is **hidden**, not disabled — the opposite of
the in-screen rule, and §7 says why: in a screen the missing action would be a mystery, in the
nav there was never anything there.
