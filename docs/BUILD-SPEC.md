...
| 4 | Assets | ✓ **gallery-grid — FIRST consumer**, carries its whole §5.3 table | ✓ §4 | ✓ | ☑ **DRIVEN 2026-08-30 (D-118 server, D-119 browser)** — `AssetsHttpTest` 12 tests / 103 assertions and `tests/E2E/assets.spec.js` **17 passing** on `KPORT=8204` (bind, owning PID, no `Server:` header, `--reset` reseed, `keel-doctor` green first). Browser: preview 96px and the tile glyph 32×32 (not the SVG default), the grid asserted to have MORE THAN ONE COLUMN at 1440 (D-113's defect, now checked on every new grid), **the hover-revealed actions proven REACHABLE BY KEYBOARD** (`:focus-within`, polled — a single read lands mid-transition, D-078), a ring on the primary link AND on the action with no nested-interactive tile, whole-page axe at WCAG 2.2 AA in BOTH themes, the 320px reflow asserted by TRYING to scroll, the alt chip's destination clicked and the field found, and four full-page captures READ. **THREE defects the captures found and every assertion passed:** a raw `{size}` placeholder, a usage count with no number, and an `<h1>` reading 'Files' where `navigation.md:62` states it is 'Assets' — all three now pinned|
| 5 | Users | ☐ | ☐ §5 | ☐ | ☐ |
| 6 | Security | ☑ | ◐ §6 — see note | ☑ | ☑ |
| 7 | Analytics | ✓ overview-stats (4th consumer) | ◐ §7 — **two of five stats named, three unbacked (DR-015)**; see note | ✓ | ☑ **DRIVEN 2026-08-29 (D-114)** — `tests/E2E/overview-stats.spec.js` on `KPORT=8202`: stat-tile geometry, whole-page axe WCAG 2.2 AA in BOTH themes, the 320 px reflow, the LINE mark asserted to be a line and not a second chart pattern, the series asserted to carry one vertex per DAY of the range (30, not the 4 days with traffic), the table's real grid tracks at 1440, the below-900 replacement, and four full-page captures read. Plus `AnalyticsHttpTest` 12 tests / 98 assertions / 0 skips and `AnalyticsDenseSeriesTest` 7 tests with its red observed |
| 8 | MCP | ☐ | ☐ §8 | ☐ | ☐ |
| 9 | Settings | ☑ | ◐ §9 — see note | ☑ | ☑ |
| 10 | Log in | ☐ | ☐ §10 | ☐ | ☐ |
| 11 | Verify | ☐ | ☐ §11 | ☐ | ☐ |
| 12 | AI chat | ☑ chrome | ☑ §12 | ☑ | ☑ (D-108; interior deferred, §0c) |
| 13 | Tasks | ◐ — see the overview-stats note | ◐ §13 — see note | ✓ | ☑ **DRIVEN 2026-08-29 (D-113)** — `tests/E2E/overview-stats.spec.js`, playground `KPORT=8201`. Stat tile 32px with an 18×18 glyph (not 300×150), axe WCAG 2.2 AA clean in BOTH themes, no horizontal scroll at 320 CSS px, plus 11 integration tests, 0 skips (D-111). Four captures read |
| 14 | Comments | ☐ | ☐ §14 | ☐ | ☐ |
| 15 | Plugins | ☐ | ☐ §15 | ☐ | ☐ |
| 17 | Setup wizard | ☐ | ☐ §17 | ☐ | ☐ |
| 18 | x402 dashboard | ◐ — see the overview-stats note | ◐ §18 — see note | ✓ | ☑ **DRIVEN 2026-08-29 (D-113)** — `tests/E2E/overview-stats.spec.js`, playground `KPORT=8201`. Stat tile and glyph geometry, axe clean in BOTH themes, no horizontal scroll at 320, the chart's real bars with its `<details>` table beside it, the chart REPLACED by that table below 900, **and the table's three grid tracks at 1440** — the defect the captures found (adaptation 95). Plus 11 integration tests, 132 assertions, 0 skips (D-112). Four captures read |
| 19 | Content model | ☑ | ◐ §19 — see note | ☑ | ☑ |
| 20 | Translations | ☐ | ☐ §20 | ☐ | ☐ |
| 21 | Blocks | ✓ **gallery-grid — SECOND consumer**, adds the wireframe preview and the group rhythm | ✓ §21 | ✓ | ☑ **DRIVEN 2026-08-30 (D-120)** — `BlocksHttpTest` 7 tests / 37 assertions and `tests/E2E/blocks.spec.js` **13 passing** on `KPORT=8205`. Server: the gallery rendered from the server, three groups each with its OWN labelled `<ul>`, the category labels asserted to come from the CATALOGUE, the usage counts asserted at 2 / 1 / none (three different answers, so a constant cannot pass), the preview and its unknown-block error. Browser: the wireframe at 120px where an asset preview is 96px, the wireframe asserted `aria-hidden` with the link still named by text, every `aria-labelledby` asserted to point at a heading THAT EXISTS, the group rhythm measured as a real gap between bounding boxes, the grid's column count, whole-page axe in BOTH themes, the 320px reflow, no placeholder on the page, and four captures READ |
| 22 | Health | ☐ | ☐ §22 | ☐ | ☐ |
...
| 44 | **Dashboard** | ◐ — see the overview-stats note | ◐ §44 — see note | ✓ | ☑ **DRIVEN 2026-08-29 (D-113)** — `tests/E2E/overview-stats.spec.js`, playground `KPORT=8201`. Stat tile 32px with an 18×18 glyph, the linked stat card as ONE anchor taking the focus ring, axe clean in BOTH themes, no horizontal scroll at 320, **and the banner link on its tint** (adaptation 96). Plus 10 integration tests (D-110). Four captures read |
...
| 52 | **Entry 32 keeps its shipped filename `taxonomy.php`**, where the manifest names `taxonomies.php` | A filename is a URL on a released product with installs in the field. Renaming it breaks every bookmark and every link entry 19 already writes | Yes — the same recorded mapping already applied to `theme.php`, `post-types.php`, `post-type-edit.php` and `license.php` |
...
| 60 | **Entry 23 keeps the xterm.js canvas and does NOT build §1's `<pre>` stream or its "real form" prompt** | The stream is the deferred engine interior (D-104, `roadmap.md` §0c), decided by the user before a line was written and for reasons the product cannot argue with: `dispatch()` buffers with `ob_start()`/`ob_get_clean()` and returns one whole string, nothing measures a command's duration, the return type carries no numeric status, and handlers run synchronously with no interrupt point. The prompt goes with it — the canvas owns the keyboard, so a second input beside it would be two prompts for one shell | Yes, and the accessibility consequence is a GAIN, not a waiver: with no HTML line targets, `accessibility.md` §7.1's sub-24px exception is never claimed on this screen and every control on it is ≥ 24px. The canvas is a labelled, focusable `role="group"`, which is more than the bare `<div>` it was |
| 61 | **`.k-terminal-canvas` is a new class rather than `.k-stream`** | `.k-stream` styles the delivery's `<pre>` panel at `--type-code`. Painting a third-party canvas with a `<pre>`'s typography would be markup claiming to be something it is not, and it withholds the one thing a canvas needs and a `<pre>` does not — a height to size itself against | Yes. Every value in it is an existing token and the frame it draws is §1's own (a `--fondo-ventana` panel inside the card, `--radio-control`, 60vh). `min-height` rather than `height` alone, because build rule 1's seventh mechanism was a track sized to its content at 320px |
| 62 | **A generic `.k-modal` / `.k-modal-window` pair is added to the component layer** | Entry 23's revalidation step is a modal by nature — a re-authentication — and `accessibility.md` §3.2 governs it, but the only overlay in the tree was `.k-palette`, named and scoped for the command palette in `klytos-shell.css`. The shipped overlay had no role, no `aria-modal`, no heading association, no focus trap, no `Esc` and no cancel: the only way out was succeeding | Yes. It is the palette's own geometry generalised under a name that says what it is, built from tokens already in the sheet, and it shipped with its `[hidden]` rule written at the same moment as the component rather than after a check caught it |
| 63 | **`trapFocus` is EXPORTED from `klytos-shell.js` as `window.KlytosShell.trapFocus`, not copied** | The revalidation dialog needs exactly the eight lines the palette and the drawer already share, and a third copy would be the duplication the project treats as a defect rather than a shortcut | Yes — generalising in place is what the reuse rule asks for. It is assigned before the file's `!shell` guard and works because a function declaration is hoisted; a new public surface, documented in `docs/reference/terminal-screen.md` with its `docs/api/INDEX.md` row |
| 64 | **The shipped `.klytos-terminal-header` bar is removed** | §1's anatomy for this template is h1 · control row · stream, with no chrome bar of its own. The bar carried a status dot, the product name and the version in six hardcoded hex values the redesign does not have | Yes, and nothing operable is lost: the dot's job is now `aria-busy` on the console plus the polite status region, and the version is one `version` command away. This is not D-076's "shipped behaviour stays" case — a decorative bar is not behaviour. The command reference, which IS behaviour, stays |
| 65 | **Entry 12 keeps the whole-response turn and does NOT build streaming, Stop, a running tool call, the inline permission confirm, the Stopped state or "Load earlier messages"** | The streaming turn is the deferred engine interior (D-104, `roadmap.md` §0c). `api/ai-chat.php` responds once with the whole result and `chat-engine.php` has no streaming path, so a partial turn cannot exist — and every one of those states is a state OF a partial turn. `getChat()` reads every message with no limit/offset, so the "Load earlier" link has no query to make and nothing to fetch | Yes, and the accessibility outcome is the one §2 asks streaming to simulate: the transcript is still `role="log" aria-live="polite" aria-relevant="additions"`, and a turn that arrives whole IS "only the finished turn announced, never each token" |
| 66 | **The `?panel=` alternate admin is retired: the four URLs 302 to the real screens and the four partials are deleted** | The screen hid the shell with five `display: none !important` rules, so Dashboard, Settings, Users and Profile were unreachable from it and were re-implemented inside it. With the shell back they are one nav click away, and a second copy of a privileged screen behind a lower gate is exactly the shape audit NEW-31 reported. **The user's decision**, taken before the rewrite: removing shipped behaviour is not a fidelity call (D-076) | Yes — and it is stronger than what it replaces. Nothing privileged is SERVED from this door at any role now; the destinations gate themselves through the central map. `AdminGateHttpTest` follows the redirect and asserts the refusal where it now lives, so the escalation is proven closed rather than moved |
| 67 | **The conversation history is rebuilt as a disclosure in the header the template DOES define** | §1's anatomy is header · transcript · context chips · composer, with no conversation list anywhere. The screen ships a whole sidebar with a list, a search field and a separate browser view, and D-076's rule holds for the fifth time. **The user's decision** among three offered | Yes. It uses the delivery's own disclosure semantics — `aria-expanded` + `aria-controls`, Esc closes, focus returns — which is entry 23's command-reference precedent, and every capability survives: new, list, search, open, delete |
| 68 | **The starters are `<button>`s, and their three sentences are the delivery's literal ones rather than "drawn from the last screen visited"** | §2 draws them as links; a link needs a destination and these have none, because the send path is a fetch. That is DR-004's finding on a different control. Nothing in the tree records a last screen visited (D-104), so the derivation is the deferred half and the three quoted sentences are the built half | Yes. A control that changes state is a `<button>` — `accessibility.md` §5.1's own rule — and each is ≥ 24px. Clicking one fills the composer; it does not send, so nothing is dispatched that the person did not read first |
...
| 95 | **Entry 18's three tables declare `grid-template-columns: minmax(0, 1fr) max-content max-content` (two tracks for the provider table), DERIVED rather than read** | §18 records no columns and no widths for the chart's data table, *Top pages* or *Top bots* — DR-006's gap arriving on a screen its table never enumerated (ADDENDUM added there). Unlike entry 25, no prototype settles it either: `Klytos Admin - Screens 3.dc.html` carries `x402Tables` as name/count/amount ROWS with an icon, not as a grid with tracks. So the tracks are content-driven — the label takes the space, each number takes what it needs — which invents no measurement and cannot disagree with a value Design has yet to write. **Replaced verbatim the moment DR-006 answers.** `minmax(0, 1fr)` and not `1fr`, because a grid track's default minimum is `auto` and a long unbroken path would otherwise push the numeric columns off the card | Yes — and it closes a SHIPPED defect rather than opening one. Without it all three fell to `klytos-components.css`'s deliberate `1fr` fallback and rendered ONE CELL PER ROW at every width, turning a 1000px viewport into a 5731px page (commit `d71a704`). Pinned by a test proven to fail on the planted defect |
| 96 | **A link inside a tinted banner takes the tint's own text colour (`currentColor`) and is underlined, not `--color-acento`** | Not a design deviation — **a defect the build shipped and the build fixed**, and the delivery states the rule itself: text on a tint is `--sobre-tinte-*` (`SPEC/color-contrast-audit.md`, `klytos-admin.css:18`), which `.k-banner--aviso` already sets on the container. The link layer's ENUMERATION names `.k-banner` so a link there would not fall back to the pre-redesign blue, and that made it the accent — which over `--tinte-aviso` composites to **3.68:1 in light**, measured out of the browser on entries 44 and 18. The underline keeps the link distinguishable without colour (WCAG 1.4.1) | Yes. `currentColor` so every future `.k-banner--*` modifier is right by construction rather than by being remembered — the enumeration trap, not repeated one component down. Pinned by a test proven to fail on the planted defect |
| 97 | **Entry 7 draws FOUR stat cards: §7's *Views* and *Visitors*, plus *Avg. views/day* and *Pages tracked* which §7 does not name** | Three of §7's five have **no source in the product** and the reason is the analytics engine's own privacy design, not a gap: *Avg. time* needs two instants and a pageview records one; *Bounce* is defined on a session and there is no session concept at all; *Agent hits* needs a user agent the engine deliberately never stores, and the only agent record in Klytos is the x402 log of agents that PAID — entry 18's *Paid requests* under another name. **DR-015.** The two added cards are shipped product that predates the redesign, and dropping shipped measured figures is not a fidelity decision (D-076, D-079) | Yes — and two cards alone would break `template-overview-stats.md` §1's own 3–5 floor, so building only §7's measurable pair was never an option either. Every figure on the row is measured; none is invented |
| 98 | **The 24h period chip is kept, where §7 names 7d · 30d · 90d** | Shipped behaviour on a released product: a filter someone may be using is not removed for fidelity (D-076's rule, D-079's precedent with the trash and custom-status chips). It is the same control in the same `<nav>`, carrying `aria-current` like the other three | Yes. §7's three are all present, in its order, and the default is its 30d |
| 99 | **The Devices card is kept, and rendered as a real table rather than the three emoji the pre-redesign screen drew** | §7's card list does not name it and the product measures it — same call as entry 18's provider card (row 94) and entry 13's *In progress* chip: built and logged, never silently dropped. The emoji had to go regardless: a device class carried by a picture alone is a label with no text, which `accessibility.md` forbids for state and is no better for a category | Yes — the card is the admin's own table component with a caption, and its percentages are the manager's own |
| 100 | **Entry 7's tables declare `grid-template-columns: minmax(0, 1fr) max-content`, DERIVED rather than read** | §7 records no widths, exactly like §18 one slice earlier — DR-006's gap on its sixteenth surface, and its ADDENDUM now covers both. Every table on this screen is label + one number, so two tracks. Replaced verbatim the moment DR-006 answers | Yes. It also carries D-113's lesson forward by construction: the assertion that a table has real tracks is now written for every consumer of this template, not just the one that shipped the defect |
| 101 | **Entry 4 is rebuilt SERVER-RENDERED, replacing a screen that rendered entirely in the browser** | Not a deviation — the deviation was what shipped. `template-gallery-grid.md` §2 states "Loading — **server-rendered**. Pagination is a link. There is no infinite scroll anywhere in the admin", and the shipped screen was `<!-- Populated by JS -->` inside a card that started `hidden`, with every filter a `<select>` filtered client-side and pagination built in JavaScript: **with scripting off it showed nothing at all.** Every control now works without a script, like the twelve screens before it | Yes, and it is the only reading of §2 that is available. The cost is stated rather than hidden: the grid/list toggle, the drag-and-drop upload and the client-side category modal are gone, and the maintenance actions became real POST forms — which also makes them work for the first time without JavaScript |
| 102 | **The `Fonts` chip is kept, where §4 lists All · Images · Video · Documents · Unused** | Shipped control (`assets.php:95` before the rewrite). Removing a filter someone may be using is not a fidelity decision — the same call as entry 13's *In progress* chip (row 87) and entry 7's 24h period (row 98). §4's five are all present, in its order | Yes |
| 103 | **The drag-over state is not built** | §2's Drag-over is a client-side event and this screen has no script. §2 itself names the substitute in the same breath — "A keyboard user reaches the same thing through *Choose files*, which is always present" — and that control is the whole mechanism here, not a fallback beside a better one. Same family as rows 72–73 (record-form's *Dirty* and *Saving*, list-table's *Loading*) | Yes — the accessible path is the ONLY path, which is stronger than the template asks for |
| 104 | **The uploading state's `<progress>` is not built** | Same reason as row 103: a determinate progress bar describes an in-flight XHR, and a form POST has no such moment. The upload's result is reported by the status line the screen already renders | Yes |
| 105 | **Entry 4's usage table declares `grid-template-columns: minmax(0, 1fr) max-content`, DERIVED rather than read** | §4 records no widths for it — DR-006's gap on its seventeenth surface, covered by the same addendum. Two tracks: context + field. Replaced verbatim when DR-006 answers | Yes |
| 106 | **Entry 21's usage count is TEMPLATES, and the tile names the unit** — "In 2 template(s)", never a bare number | §21 draws a usage count and does not say what it counts. The product holds the relationship at one remove: a page template lists the blocks it renders, and a block in ONE template can render on FIFTY pages. On a tile with a bare number those two answers look identical and mean opposite things to someone deciding whether a block is safe to change. Templates is the direct fact, it is one traversal for the whole screen, and a label that names its unit cannot be misread. **DR-016** asks which §21 intends | Yes — the figure is measured rather than invented, and what it measures is stated instead of implied |
| 107 | **Entry 21's category labels move from hard-coded English to catalogue keys** | Not a design deviation — a shipped defect. The screen carried `'Structure'`, `'Content'`, `'Interaction'`, `'Social Proof'` and `'Custom'` as literals on a product with twenty catalogues; the same defect as the Forms plugin's Spanish sidebar (D-116), from the opposite side. A fallback to the raw id keeps a plugin's own category readable | Yes |
