...
| 4 | Assets | ✓ **gallery-grid — FIRST consumer**, carries its whole §5.3 table | ✓ §4 | ✓ | ◐ **server tier only, 2026-08-30 (D-118)** — `AssetsHttpTest` 12 tests / 103 assertions / 0 skips over a deterministic library (2 images, one with alt text and one without · 1 video · 1 document · exactly 1 in use). Driven: the grid asserted to arrive IN THE HTML with its four tiles and `Populated by JS` asserted ABSENT; each kind chip asserted to select a DIFFERENT set by count; `Unused` excluding exactly the asset in use; the *No alt text* chip on exactly ONE tile and asserted to be an anchor at `#alt`; delete disabled on exactly one with the reason and the count in its `aria-label`; **a posted delete of an in-use asset refused by the SERVER**; both empty states asserted to be different sentences; a genuinely refused CSRF. **The browser tier and the captures are OWED and NOT claimed** |
| 5 | Users | ☐ | ☐ §5 | ☐ | ☐ |
