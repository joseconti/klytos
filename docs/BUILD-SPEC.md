...
|---|---|---|---|
| Tables | Real `<table>` + the complete explicit role set, element by element | §2.1 | ☐ |
| Not tables | Assets grid, Blocks/Templates galleries, sidebar nav — `role="list"` / `<nav>` | §2.2 | ☐ |
...
| Components ×12 | Name/role/state per §5.1–§5.12 (button, badge, chip, switch, checkbox, radio, field, progress, stat card, dock, palette, the rest) | §5 | ☐ |
| Switch vs checkbox | Immediate effect = `role="switch"`; needs Save = checkbox — **decided per screen in the manifest** | §5.4 | ☐ |
| Errors | `aria-invalid` + icon + word + sentence; summary in `role="alert"`, focus moved | §6 | ☐ |
...
| Target size | 24 × 24 via `.k-hit-24`; the drawing does not change size | §7 | ☐ |
| Zoom & reflow | 200 % at 1280×800 and 400 % (320 CSS px); no page-level horizontal scroll | §8 | ☐ |
...
| Reduced motion | The `prefers-reduced-motion` block is a rule; the sync dot stops pulsing and stays lit | §9 | ☐ |
...
| 6 | *(reserved — every further adaptation is appended here at the moment it is made, with its evidence)* | | |
