...
| **5 of 6 — the form screens, BATCH B: entry 25 (Consent)** (2026-08-10, D-092) | observed | `tests/E2E/consent.spec.js` — **22 tests**, all passing, on `KPORT=8181` (bind confirmed, owning PID `php83 93214` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028); whole tier re-run at **275 passing** (was 253). Plus `tests/Integration/ConsentDeclarationsTest.php` — **5 tests** over the reader this screen exposed as broken. This is the ONLY screen in the build whose manifest entry reaches OUT of the admin, so the run drives TWO surfaces: `consent.php`, and the delivered `installer/core/assets/consent-manager.js` served into a real page through route interception at an `/installer/admin/` URL (D-078's technique, because the thing under test is a delivered asset and inventing a product URL for it would be inventing product). Driven on the screen: the two backed cards plus the preview each with its own `<h2>` and exactly one `<h1>`, **the absence of the deferred Acceptance stats card asserted** so it cannot be invented later from a number nobody measured, the equal-prominence rule asserted to be a `<p>` **and to contain no control at all**, the preview's Reject and Accept compared by class list AND by box, weight and colour read out of `getComputedStyle`, §2.1's explicit role set counted rather than sampled with the four-track grid read back from the browser, the caption's counts compared against what the fixture read back through the product's own audit, the empty state as one row spanning all four columns, both exports answering with a real attachment, persistence checked by a **fresh GET**, the server-side refusal reached by stripping `novalidate` (a client-only refusal is not a refusal), `aria-describedby` asserted hint-then-error, the summary taking focus, the two-step confirm driven **with JavaScript actually disabled**, and axe at WCAG 2.2 AA over populated and empty states in **both themes** — **scanning the WHOLE PAGE, not `#main`** (L-037). Driven on the shipped banner: `role=dialog` + `aria-modal`, `aria-labelledby` resolving to a real `<h2>`, the two choices compared the same way as the preview's, focus **polled** into the dialog and Tab/Shift+Tab trapped six times in each direction, Esc asserted to record a CHOICE (`getConsentState()` non-null, analytics and marketing false) rather than to dismiss, the configured 90-day duration read out of the real cookie's expiry, and axe with the banner open. PHP tier 335 → **340 tests / 1643 assertions**, 0 skips | **red observed:** `Failed asserting that actual size 0 matches expected size 1` — and getting to that red was the work. The tests were written first in `tests/Unit/` and failed with `App::getOptionsManager(): Return value must be of type OptionsManager, null returned`, which is the absent App and not the defect; `ConsentManager::getConfig()` reads through the options layer, so the class is not unit-testable for the same reason authorization is not. Moved to the integration tier, the red names the defect itself (**L-041**). **THE LARGEST FINDING IS NOT ABOUT THIS SCREEN:** `getPluginDeclarations()` had been returning NOTHING on every install since the feature shipped — `StorageInterface::list()` yields RECORDS and this one method read them as IDS, so `read()` threw on every record and a bare `catch ( \Throwable ) { continue; }` swallowed every throw. The cookie audit, both exports and both MCP tools all reported that nothing was declared, whatever was declared. Fixed test-first with the skip **narrowed** rather than deleted, and **proven by planting the original loop back**: all five went red together, then restored byte-identically. A second defect rode along in the same class — `sanitizeCookies()` guarded its `??` on the CONDITION and read the key raw in the true branch, storing `null` for every declaration that omits `type`. **THE SHIPPED BANNER MET NONE OF §10.4's FIVE PROPERTIES** — no `aria-modal`, no heading, no focus move, no trap, no Esc, and its reject-equivalent drawn as `cm-btn-secondary` beside a `cm-btn-primary` accept, which is the colour trick the rule names. All five built and each driven; the prominence pair **proven to FAIL on the planted original class**, then restored byte-identically. Two further shipped fields that did nothing were closed in passing: `cookie_days` never reached the library, and the banner's words were hardcoded Spanish against D-006. **ONE CONTRAST DEFECT, the build's own and therefore FIXED not registered:** the preview's body text at **4.46:1 light** (DR-005 gap 2's pair on a fourth surface) — no delivered file states that control's colour, so this is D-090's call exactly. Now 14.79 / 15.29, proven red on the planted original. **One save that deleted data it does not edit**, fixed and pinned: the shipped screen rebuilt `categories` from a field it never rendered, wiping every MCP-created custom category on every save. **One thing checked and found NOT to be a defect:** `consent.php` has no in-file capability call because it is mapped centrally in `klytos_admin_gate_map()` (S-07's fix). `keel-verify` **21 checks: 16 pass, 5 warnings**; lint on `consent.php` **0 errors / 0 warnings**; `docs/api/INDEX.md` **1005 → 1011** — and the INDEX check caught my own mistake there, both filters written into the Actions section until the count and parity checks failed (L-038 a second time) | `tests/E2E/artifacts/test-results/consent-*/trace.zip` — in particular `consent-The-shipped-banner-*Reject-all-is-the-same-component*/trace.zip` (the strictest rule, measured), `consent-The-shipped-banner-*Esc-rejects-non-essential*/trace.zip` (Esc as a choice) and `consent-Entry-25-*removing-a-declaration-with-no-JavaScript*/trace.zip` (the confirm driven with JS off) |
| **Rework — entry 41 (Logs) against DR-007's answer** (2026-08-10, D-094) | n/a — out of scope | `tests/E2E/logs.spec.js` — **39 tests** (was 30), all passing, on `KPORT=8183` (bind confirmed, owning PID `php83 68548` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028); whole tier re-run at **284 passing**. All eight items `roadmap.md` §0e owes are driven: the per-line copy button asserted **ABSENT** (its return would revoke §7.1's grant), `Copy line` revealed with the selection and copying the line AS STORED (read back out of the clipboard, and asserted to contain `[ERROR]` so "as stored" means the record and not the message), `Copy all` labelled with its count and driven **with a level filter applied** so the count it names is proven to be the count it copies, the selected line's role colours read out of the browser and asserted to collapse to ONE colour, the 3px inset bar, `↑`/`↓`/`Enter` driven with the **real keyboard** as §7.1 condition 3 requires, and `.k-hit-24` asserted absent. Plus a new test asserting **all four of §7.1's conditions**, because a disabled `target-size` rule with an unchecked grant is just a disabled rule. PHP tier **340 / 1643**, 0 skips | **`n/a — out of scope`, the honest value:** this slice added no pure function of its inputs — it is markup, CSS and a keyboard handler. **THE UN-PINNING WAS THE FINDING.** Re-enabling `color-contrast` on the stream pass, which the old comment itself said would happen when Design answered, exposed a THIRD composition the blanket exemption had been covering: `--texto-sutil` on a **tint** over `--fondo-ventana` measures **3.88 / 4.18** (peligro) and **3.94 / 3.73** (aviso) — below AA in both tints and both themes, recomputed independently and agreeing with axe's 3.93. It is the delivery's (§1 specifies both halves), so it is registered as **DR-009**, excluded by selector scoped to the UNSELECTED state, and **pinned as floors at the measured values** rather than at 4.5. **One test defect of my own:** the four-conditions sweep reported the Follow switch at 38×22, but §7's table says the switch's target is its 38×24 LABEL ROW — the test was measuring the control. **One cross-tier defect I introduced last session, surfaced only now:** Consent's save calls `buildAll()`, which writes the site into the repository root, and `AdminGateHttpTest` forbids exactly that for NEW-04 — so an E2E run made the PHP suite fail on code that was fine (**L-025's shape**). The consent fixture now cleans it up, never touching `.htaccess`. **Every change proven by planting the defect back** — the base colour reverted and the copy button restored turned four tests red together, then both files restored byte-identically. **One transient recorded rather than hidden:** the first whole-tier run failed `content-model`'s 320px reflow test; it passes in isolation and on the immediate re-run (284/284), so it is order-dependent flake. `keel-verify` **21 checks: 16 pass, 5 warnings**; lint on `logs.php` **0 errors / 0 warnings** | `tests/E2E/artifacts/test-results/logs-*/trace.zip` — in particular `logs-*four-conditions*/trace.zip` (§7.1's grant checked rather than claimed), `logs-*operable-from-the-keyboard*/trace.zip` (condition 3 driven with the real keyboard) and `logs-*Copy-all-follows-the-FILTER*/trace.zip` |
| **5 of 6 — the form screens, BATCH B: entry 9 (Settings)** (2026-08-10, D-096) | observed | `tests/E2E/settings.spec.js` — **35 tests**, all passing on `KPORT=8184` (bind confirmed, owning PID `php83 47816` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028). Driven per SECTION and per STATE: the H1 stays *Settings* on all five while the section is the `<h2>` (asserted by TAG NAME, not by looking); the breadcrumb's last two crumbs read `Settings` then the section; every nav item is a real URL and clicking one is a page load; an unknown `?section=` resolves to the first rather than failing; exactly ONE Save exists and it is outside `<main>`, reaching the form by `form=`. **The re-partition is driven, not reasoned about**: SMTP host and port saved on Email, then Site saved, then both re-read through a FRESH GET (D-088). Validation both directions — empty name, `abc` port, malformed sender, half-filled language row, each with its summary/`aria-invalid`/`aria-describedby` order — plus *nothing is validated on load* across all five. DR-002 driven in BOTH halves: the checkbox moves the setting and survives a fresh GET, and the Dashboard is asserted to carry a LINK and NO toggle. The 900px breakpoint read out of the browser (`flexDirection` column → row) with the `aria-label` asserted to SURVIVE the layout change. axe at `wcag2a/2aa/21a/21aa/22aa`, whole page (L-037), **5 sections × 2 themes = 10 runs, plus the ERROR state** — resting states only is how a component ships below AA (D-091). Focus order checked as DOM order AND as a real `Tab` landing. PHP **345 / 1651**, 0 skips (was 340 / 1643) | **red observed:** `Failed asserting that an array has the key 'encryption_key_backed_up'.` plus `Failed asserting that null is false.` beside it — `FAILURES! Tests: 4, Assertions: 6, Failures: 2`, seen before the fix existed. It names the ABSENT BEHAVIOUR, a key missing after a round trip, and not a missing class or a typo — the distinction L-041 paid for. Green after one name was added to `set()`'s allow-list: `OK (4 tests, 7 assertions)`. **Six shipped defects closed, five of them found by reading or driving rather than by a report.** THE FINDING: `SiteConfig::set()` had been silently dropping `encryption_key_backed_up` since the feature shipped — two surfaces wrote it, `bootstrap.php` reads it as the condition of an **undismissable system error notice**, so every install carried a permanent red banner no control in the product could clear. L-041's shape exactly: it did not break, it agreed with you. Also closed: a default-language select hard-coded to es/en/ca/fr whatever the language list said (a German site could not pick German, and saving rewrote the stored value); a half-filled language row dropped without a word; `smtp_port` cast to `int` so `abc` became port 0; the seven devbar toggles rendered only once developer mode was already on, so they could not be set in the same save that enables it (§2 forbids hiding a disabled control); and a notices card that vanished when empty, so *no notices* and *broken* looked identical. **One spec miss of my own, caught only by driving:** §4 wants the word *Required* in the hint and it was absent — found because two tests failed for the WRONG reason, Chromium's own constraint validation refusing the submit so the server path they name never ran. Repaired rather than relaxed: the word added in all 20 catalogues, and the helper now clears `noValidate` with the reason written down. **One test moved rather than deleted:** `AdminGateHttpTest`'s two Dashboard rows guarded a branch that no longer exists, so the coverage moved into a test pinning BOTH halves of the move — gated where it landed, AND no copy left behind. `keel-verify` **21 checks: 16 pass, 5 warnings**; lint on the touched files **0 errors**, core+admin baseline **158/423 — DOWN from 191/488** | `tests/E2E/artifacts/test-results/settings-*/trace.zip` — in particular `settings-saving-one-section-*/trace.zip` (the re-partition proven lossless through a fresh GET), `settings-the-Dashboard-warns-*/trace.zip` (DR-002's second half) and `settings-axe-is-clean-on-the-error-state-*/trace.zip` |
| **5 of 6 — the form screens, BATCH B: entry 37 (x402 settings)** (2026-08-10, D-098) | observed | `tests/E2E/x402-settings.spec.js` — **34 tests**, all passing on `KPORT=8161` (bind confirmed, owning PID `php83 29238` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028); whole tier re-run at **353 passing** (was 319). Plus `tests/Integration/X402ConfigUpdateTest.php` — **5 tests** over the manager this screen exposed as broken. Driven: five cards with exactly one `<h1>`; the toolbar Save reaching a form it sits outside of by `form=`; every control checked for a visible `<label for>` by walking the DOM rather than by sampling; the wallet mono, editable and copyable; price validation both directions with summary, `aria-invalid`, `aria-describedby` order and the summary taking focus; a refused save proven to store nothing; the licence pair round-tripping through a **fresh GET** (D-088) and an unknown licence type posted by hand proven not to reach storage; the built-in agent list asserted `readonly` and NOT `disabled`; the collection's empty state, add, duplicate refusal, built-in duplicate refusal, per-row accessible name, and **removal**; both toggles asserted to be checkboxes and NOT `role="switch"`; the whole add/remove/save trio driven **with JavaScript actually disabled**; 320 CSS px measured for horizontal overflow on the PAGE; and axe at `wcag2a/2aa/21a/21aa/22aa` over the WHOLE page (L-037) in **3 states × 2 themes** — at rest, in the error state and with a populated collection. PHP **345 → 350 tests / 1660 assertions**, 0 skips | **red observed:** `Failed asserting that two arrays are identical. --- Expected +++ Actual @@ Array &0 [ 0 => 'AlphaBot', + 1 => 'BetaBot', ]` — `FAILURES! Tests: 5, Assertions: 9, Failures: 3`, seen before the fix existed. It names the defect itself, a removed agent surviving the save that removed it, and the two "other direction" tests passed from the start, which is what made the red trustworthy. **THE FINDING: `X402\Config::update()` merged lists INDEX BY INDEX**, so `['A','B']` updated with `['A']` came back `['A','B']` and every list setting was one-way. `custom_bot_user_agents` is the list that decides who meets the paywall: an agent added by mistake could be added by anyone with `site.configure` and removed by nobody, through this screen or over MCP. **A SECOND FINDING IS A DISCLOSURE:** the shipped screen wrote the stored provider secret into the password input's `value=`, so a live Stripe key was in the admin page's HTML on every load — readable in view-source, in a proxy log and in any cached copy. The field is no longer populated and a blank post means *keep*, which the hint says in words. **Also closed:** the provider could never actually be changed (the new provider's required fields were validated against a form that had rendered the OLD provider's); `default_price_usd` stored whatever was typed, so `abc` became the price of every protected page (D-096's `smtp_port` defect on another screen); the built-in list was `disabled` rather than `readonly`; and a stored network the active provider no longer supports was silently rewritten by the next save. **One defect of my own, found by driving and proven by planting it back:** a refused save re-rendered the STORED values, so the screen showed `0.075` beside an error saying the price is not a number. **And a check that was blind to a whole directory:** `installer/lang/x402/` was covered by no parity check, and four of its twenty catalogues had been two keys short since the module shipped. `keel-verify`'s locale check now walks `installer/lang/*` too — **120 files across 6 sets → 140 across 7**, count confirmed to have moved (L-038), and proven to FAIL on a gap planted in the newly covered root before being trusted. `keel-verify` **21 checks: 16 pass, 5 warnings**; lint on the touched files **0 errors**, core+admin baseline **151/423 — DOWN from 158/423** (D-025 records 191/488); `docs/api/INDEX.md` **1012 → 1024** | `tests/E2E/artifacts/test-results/x402-settings-*/trace.zip` — in particular `x402-settings-AN-AGENT-CAN-BE-REMOVED-AGAIN-*/trace.zip` (the defect this slice fixed), `x402-settings-NO-PROVIDER-SECRET-IS-WRITTEN-*/trace.zip` (the disclosure) and `x402-settings-with-JavaScript-disabled-*/trace.zip` |
| **5 of 6 — the form screens, BATCH B: entry 26 (Privacy)** (2026-08-10, D-099) | observed | `tests/E2E/privacy.spec.js` — **23 tests**, all passing on `KPORT=8161` (bind confirmed, owning PID `php83 73251` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028), plus the new `tests/E2E/fixtures/reset-privacy.php`, which builds a **disposable data subject** through the product's own writers because every erasure this screen performs is irreversible and a seeded role would burn the rest of the tier. Whole tier re-run at **376 passing** (was 353). Driven: two cards with exactly one `<h1>`; the tabs proven gone and each card proven unable to retarget the other; a visible `<label for>` on every search control **and the absence of a placeholder** asserted; the export section list as a real `<table>` with `role=table`/`rowgroup`/`row` and a `<th role="rowheader">`; a JSON download whose filename carries the subject; the two-step confirm with the section list proven ABSENT between the steps and nothing erased by arming (checked with a **fresh GET**, D-088); **zero browser dialogs** asserted by counting `page.on('dialog')` across both the confirm and the empty-selection paths; an empty selection proven to produce a field error and NOT to arm; the owner's retained section stated in words with no destructive control offered at all; the whole search/arm/confirm chain driven **with JavaScript actually disabled**; and axe at `wcag2a/2aa/21a/21aa/22aa` over the WHOLE page (L-037) in **4 states × 2 themes** — at rest, populated, in the error state, and showing an erasure result. PHP **350 tests / 1660 assertions**, 0 skips (unchanged — the defect is in the screen, which only the browser tier reaches) | **red observed:** `the erasure reported no per-section outcome at all — Locator: getByText('Form Submissions') … Error: element(s) not found`, seen against the SHIPPED markup before a line of the rewrite existed, with `.alert-success` visible in the same run — which is what made it a REPORT defect and not a failure to erase. Its first red was the wrong one and is recorded rather than hidden: an unscoped `main form button` matched the SHELL's theme form, so the search never ran (L-042). **THE FINDING: the per-section erasure result table had never rendered on any install since it shipped.** `$foundUser` was assigned only in the `search_user` branch while the results block sat nested inside `if ( $foundUser !== null )`, so the `erase_data` POST always found it null; somebody who had just irreversibly erased another person's data saw one green sentence and nothing else. **Two more defects were hiding BEHIND it**, both unreachable while the table never drew: the rows printed the section ID (`core:form_submissions`) and never the label, and the manager returns status `deleted` for form submissions while the catalogue had no `privacy.deleted`, so the commonest erasure of all would have rendered its own key. **Also closed:** the search fields were placeholder-as-label with no `<label>` at all; a retained section was explained only by a `title` attribute on a padlock icon; and the empty-selection guard was a JavaScript `alert()`. **One defect of my own, found by driving:** the section list rendered only when at least one section was erasable, so on the owner — where every section is retained — the screen said "nothing can be erased" and never said WHICH or WHY. **And L-044, the lesson of this slice:** planting back the line whose comment said "THE FIX" left all 22 tests GREEN, which established that the un-nesting is the fix and the re-resolution is a separate improvement — one that had no test at all until this run added it. Both were then planted separately and each failed its own owning test, and the original defect was reconstructed exactly (subject nulled AND table re-nested) and failed "an erasure reports what it did". Restored byte-identically after each. `keel-verify` **21 checks: 16 pass, 5 warnings**; lint on the touched files **0 errors, 0 warnings**, core+admin baseline **148/420 — DOWN from 151/423 on both counts** (D-025); `docs/api/INDEX.md` **1024 → 1027** | `tests/E2E/artifacts/test-results/privacy-*/trace.zip` — in particular `privacy-an-erasure-reports-what-it-did-section-by-section-chromium/trace.zip` (the defect this slice fixed), `privacy-a-partial-erasure--*/trace.zip` (the owner's retained section) and `privacy-with-JavaScript-di-*/trace.zip` |
| **5 of 6 — the form screens, BATCH B: entry 27 (Profile)** (2026-08-10, D-100) | observed | `tests/E2E/profile.spec.js` — **26 tests**, all passing on `KPORT=8163` (bind confirmed, owning PID `php 16495` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028). Driven as a DISPOSABLE ACCOUNT — `tests/E2E/fixtures/reset-profile.php` — because this screen edits the password of whoever is logged in, so driving it as a seeded role would burn `passwordFor()` for every spec after it (D-099's rule from the other side). Three cards × the states §2 names: at rest, saved, field-level error, form-level summary, a refused CSRF post, a rejected password, a refused `javascript:` URL, a duplicate email, the theme switch, and the whole screen with **JavaScript disabled**. axe over the WHOLE page (L-037) at rest, in the error state and in the saved state, in **both themes** = 6 scoped runs | **red observed:** `the save was reported as failed and the identity half had already been written — Expected: "Profile" · Received: "Renamed"` — the shipped handler wrote the identity fields and only then rejected the password, so a save reported as FAILED had half happened. Also reproduced first: `locator('main [role="alert"]:not(#k-live-alert)') — element(s) not found` (a refused CSRF post said nothing at all) and `label × 4, select-name × 1, color-contrast × 1` (not one control on the screen had a programmatic label). A fourth defect came from the read-back duty rather than a test: `console.error: Loading the image 'https://www.gravatar.com/avatar/…' violates the following Content Security Policy directive: "img-src 'self' data:"` — the avatar preview has been unloadable on every install since it shipped. **All four fixes were planted back and each failed its own owning test**, restored byte-identically after each. PHP **350 / 1660**, 0 skips; browser tier **402 passing**; lint on the touched files 0 errors, 0 warnings, core+admin baseline **147/412 — down from 148/420** | `tests/E2E/artifacts/test-results/profile-*-chromium/` |
| **5 of 6 — the form screens, BATCH B: entry 28 (Licence)** (2026-08-10, D-101) | observed | `tests/E2E/licence.spec.js` — **22 tests**, all passing on `KPORT=8171` (bind confirmed, owning PID `php83 34456` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028). Driven against `tests/E2E/fixtures/reset-licence.php`, which writes the licence record one level BELOW the product's own writer and says so: `License::activate()` is the only writer and it posts to `plugins.joseconti.com`, so a tier that used it would be measuring the vendor's uptime. Four licence states — none, valid, expired, revoked — × the states §2 names: at rest, the empty collection, a field-level error, a form-level summary, a refused CSRF post, the readonly key, the degraded delta, the grace period, and the whole screen with **JavaScript disabled**. The status bar's one fact is asserted on the DASHBOARD, not only here, because a bar that carries it solely on the licence screen shows it to the one person already looking. axe over the WHOLE page (L-037) in **4 states × both themes** plus the ERROR state = **9 scoped runs** (D-091) | **red observed:** `TITLE license.title — Klytos Admin`, with `main` reading `license.title / license.status / license.status / license.inactive / license.plan / license.domain / license.activated_on / license.last_check / license.key / license.activate` — every label on the screen was a literal catalogue key, because the screen called the root `license.*` and the catalogues defined `plugin_license`, a root nothing in the tree referenced (L-046). Also reproduced first: `locator('main [role="alert"]:not(#k-live-alert)') — element(s) not found` (a refused CSRF post said nothing at all) and `Expected: /Enter a licence key/ · Received: "license.key is required."` (an error message built by concatenation, reachable by no catalogue). A fourth defect came from DRIVING the new work: the status bar's link measured **4.42:1 in dark** (`#5b8def` on `#2a2a2e`) as the first consumer that filter has ever had — corrected and re-measured at **6.92:1 dark / 5.57:1 light**. **FOUR fixes were planted back — the CSRF branch, the catalogue root, the status-bar link colour and the concatenated message — and each produced a red owning its own test** (the root plant cascaded to five, correctly: a broken root breaks every label). Restored with `git checkout --` after each, which is safe here and only here because **the slice was committed BEFORE the first plant** — L-045's own carve-out, applied deliberately rather than relied on. PHP **350 / 1660**, 0 skips; browser tier **424 passing** (was 402); lint on the touched files 0 errors, 0 warnings, `license.php` 2 warnings → 0, core+admin baseline **147/409 — warnings down from 412** | `tests/E2E/artifacts/test-results/licence-*-chromium/` |
| **5 of 6 — the form screens, BATCH B: entry 32 (Taxonomies), the FORM half** (2026-08-10, D-102) | observed | `tests/E2E/taxonomy.spec.js` — **12 tests**, all passing on `KPORT=8173` (bind confirmed, owning PID checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028). Driven against `tests/E2E/fixtures/reset-taxonomy.php`, which builds a post type carrying BOTH kinds of taxonomy the product stores — hierarchical and flat — because the form's fourth field exists only for one of them, and which clears pre-existing terms through `deleteTerm()` rather than by unlinking storage. States driven: at rest on each kind, the EMPTY taxonomy, a field-level error with its `aria-describedby` link, a form-level summary, a refused CSRF post, the armed delete, the completed delete, and the whole screen with **JavaScript disabled**. Persistence is read back on a FRESH GET and then out of storage through the manager, never with `page.reload()` (L-035). axe over the WHOLE page (L-037) in **4 states × both themes = 8 scoped runs** (D-091) | **red observed:** `getByTestId('taxonomy.field.parent') — element(s) not found` — the Parent field had never existed, although `addTerm()` accepts `parent` and the handler read `$_POST['parent']`, so hierarchy has been unreachable from the admin since the screen shipped. Also reproduced first, each against the shipped markup through its own selectors: `main` reading `E2E categories — common.add / Name / common.slug / Description / common.add` (three labels were literal catalogue keys, L-046); `ALERTS: 0` (a refused CSRF post said nothing at all — the THIRD screen with the identical defect); `Term 'e2e-parent-term' already exists in taxonomy 'e2e-tax-cat'.` (the manager's raw English exception, naming internal ids, shown in all 20 locales); and `DIALOG: "Are you sure you want to delete this?"` (a browser `confirm()`, which §2 forbids by name). A sixth defect came from the FIXTURE rather than the screen — `PostTypeManager::delete()` has never deleted a post type's terms on any install — with its own unit red: `A term outlived the post type that owned it`. A seventh came from DRIVING the new work and was this build's own: `.k-code` paints a sunken ground, so the slug line inside an elevated card measured **4.46:1 in light**; corrected to the bare `<code>` entry 19 uses and re-measured out of the browser at **4.85:1 dark / 5.07:1 light**. **FIVE fixes were planted back and each produced a red owning its own test** — the CSRF branch, the Parent field, the `.k-code` class, a catalogue key, and a bad key against the new `keel-verify` check. Restored with `git checkout --` after each, safe because **the slice was committed BEFORE the first plant** (L-045). PHP **352 / 1664**, 0 skips (was 350/1660); browser tier **436 passing** (was 424); `keel-verify` **23 checks: 17 pass, 6 warnings** (was 21/16/5 — the count was confirmed to move, L-038); lint on the touched files 0 errors, 0 warnings | `tests/E2E/artifacts/test-results/taxonomy-*-chromium/` |
| **5 of 6 — the form screens, BATCH B: entry 24 (Webhooks), the FORM half** (2026-08-10, D-103) | observed | `tests/E2E/webhooks.spec.js` — **15 tests**, all passing on `KPORT=8175` (bind confirmed, owning PID `php83 89838` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028). Driven against `tests/E2E/fixtures/reset-webhooks.php`, which seeds through `WebhookManager::create()` — the real validation, the real hooks, the real generated secret (L-005) — and then rewrites each record's `url` to an address SafeHttp refuses, so the screen's Test control never opens a socket. That is not a fictional state: it is exactly the case `sendHttpPost()`'s own comment describes, and the fixture says so. States driven: at rest, the EMPTY collection, a field-level error with its `aria-describedby` link, a form-level summary, a refused CSRF post, an endpoint the manager refuses, the armed delete, the completed delete, the one-time secret handover, and the auto-disabled endpoint with its failure count. Persistence is read back on a FRESH GET, never with `page.reload()` (L-035). axe over the WHOLE page (L-037) in **both themes**. One claim is deliberately NOT in this tier: refusing a well-formed URL SafeHttp will not fetch writes an ERROR line by design, and the read-back duty is right to fail a browser test that produces one — so it is asserted in `tests/Integration/WebhookAdminRefusalHttpTest.php` instead | **red observed:** `Error: Call to undefined method Klytos\Core\WebhookManager::sendTestEvent()` — and behind it the finding the red exists for, pinned by a test that passes both before and after: `dispatch( test.ping ) attempted a delivery` is asserted to be FALSE, because `test.ping` is subscribable nowhere and both test controls therefore reached **no endpoint on any install** while reporting success (L-041's shape). The three browser-tier claims were proven by PLANTING THE SHIPPED DEFECT BACK, each producing a red owning its own test: removing the CSRF `else` → `THE REPRODUCTION — a bad CSRF token reports the refusal`; making the first delete click write instead of arm → `THE REPRODUCTION — delete is a two-step inline confirm, never a browser dialog`; dropping `form=` from the event checkboxes → `the two cards post as ONE record`. Restored with `git checkout --` after each, safe because **the slice was committed BEFORE the first plant** (L-045), tree clean after all three. Three TEST defects were separated from the product, which was again the work: `exclude( KNOWN_DELIVERY_GAPS )` reads an array as a CHAINED selector and silently excludes nothing, so both axe runs failed on DR-005 addendum 2's already-registered sidebar pair (the trap content-model.spec.js had already recorded); the catalogue-key SHAPE matcher flagged `x402.payment` because it matches a PREFIX of a name the screen prints on purpose; and a stale re-seed inside the empty-state test named ids that no longer existed. A fourth was in the new PHP test — it asserted the refused draft was absent when returning it is the behaviour the screen wants. Four invented CSS classes were caught before the file was ever run, by grepping the components sheet: the real badge modifiers are `k-badge--exito` / `--aviso` / `--peligro`, and `.k-collection-actions` is already a flex row. PHP **360 / 1679**, 0 skips (was 352/1664); browser tier **451 passing** (was 436); lint on the touched files 0 errors, 0 warnings | `tests/E2E/artifacts/test-results/webhooks-*-chromium/` |
| **6 of 6 — the specialised screens: the SECURITY fix stage 6's survey found first** (2026-08-10, D-104) | observed | `tests/E2E/terminal.spec.js` — the file opens with a reproduction rather than with the redesign, because the per-screen survey found a live cross-site scripting hole in entry 23 before a line of the redesign existed. Driven on `KPORT=8176` (bind confirmed, owning PID `php83 5231` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028); whole tier re-run in one pass at **452 passing** (was 451). It needed a fixture of a shape no previous screen has: `tests/E2E/fixtures/reset-terminal.php`, which turns the SEEDED OWNER's second factor on and off again, because a disposable subject is impossible here — `terminal.php` refuses to render without 2FA active and `terminal.access` is granted to `['owner']` alone, while the owner role is UNIQUE (`create()` refuses a second, `update()` refuses to promote one). So the spec computes a real TOTP (HMAC-SHA1, 30s, 6 digits, matching `two-factor.php:1105-1120`) from a per-run secret written to the gitignored `artifacts/`, and logs in THROUGH the second factor; `--off` is idempotent and runs in `afterAll` whether the body passed or not, because an owner left needing a code would strand every other spec in the tier. Verified after the run: 2FA off, secret file gone, tree clean | **red observed:** `locator('#cmd-panel-list').locator('.k-xss-probe-description') — 14 × locator resolved to 1 element, unexpected value "1"` — seen against the SHIPPED code, so the red is the vulnerability itself and not a plant-back. **THE FINDING: `populateCommandPanel()` built the command reference panel by string concatenation and assigned it to `panel.innerHTML`** — `cmd.usage` into a `title=` attribute, `cmd.name` and `cmd.description` into element bodies. Those values are served by `api/terminal-autocomplete.php` from `getCommandsMetadata()`, which passes the command table through the **`terminal.commands` filter**, so **any installed plugin could execute script in the owner's admin** through a command description. The repository is public, so it is fix-then-disclose and never a public issue first. The fix is a change of MECHANISM, not an escaper: `textContent` and `setAttribute` cannot produce an element or an attribute from a value whatever it contains, and cannot be forgotten at one interpolation while looking right at the others. The probe is a `<span>` and NOT an `<img onerror>` on purpose — the read-back duty fails a test on any console error or failed request, so the classic payload would go red for the wrong reason and STAY red after the fix (L-042 applied to a test's own failure). Three values are probed separately — description, name and the attribute break-out — so a partial fix still fails; and the mirror is asserted too, that the value is still SHOWN as text, so a fix that silently dropped the description could not pass. Lint on the touched files **0 errors**, the one warning pre-existing and on a line this change did not touch | `tests/E2E/artifacts/test-results/terminal-*-chromium/` |
| **6 of 6 — the specialised screens: entry 2 (Page editor), the CHROME** (2026-08-10, D-105) | observed | `tests/E2E/page-editor.spec.js` — **25 tests**, all passing on `KPORT=8176` (bind confirmed, owning PID 5231, no `Server:` header, `--reset` seeded). Driven: the shell the screen used to hide with four `!important` overrides · the single `<h1>` = the record and the non-link last crumb · the canvas's labelled section and its **real** slug control (it was a HIDDEN input) · the inspector as an `<aside>` of `<h3>` + `aria-expanded` disclosures that is never blank · the toolbar's two actions, secondary then primary, and the Save owning a form it sits outside · the save-state slot in both its resting readings · the `role="alert"` autosave panel proven not to paint while hidden · the whitespace-title refusal with its focused summary · publishing announcing the URL as a link · the slug round-tripping through a **fresh GET** · the sheet at 1100 (focus in, Esc out, focus returned) and the column at 1440 · 1.4.10 at 320 CSS px · axe WCAG 2.2 AA over the WHOLE page in both themes, resting and with the sheet open · and DR-005 gap 2's new surface pinned at its measured 4.23:1. | **red observed:** `Uncaught TypeError: Klytos\Core\PageManager::create(): Argument #1 ($data) must be of type array, string given, called in installer/admin/page-editor.php on line 111` — read out of the product log by the read-back duty, before the fix. Creating a page from this screen was fatal on every install. | `tests/E2E/artifacts/test-results/page-editor-*-chromium/` |
| **6 of 6 — the specialised screens: entry 23 (Terminal), the CHROME + NEW-33** (2026-08-11, D-107) | observed | `tests/E2E/terminal.spec.js` — **12 tests** (was 1), all passing on `KPORT=8181` (bind confirmed, owning PID `php83 48718` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028); **whole tier re-run at 488 passing in 13.7 minutes** (was 477 — the eleven new tests, and every other spec still green after `klytos-shell.js` gained an export that every admin page loads). Driven: the shell's single `<h1>` · the console as a labelled, focusable `role="group"` whose `aria-label` is asserted NON-EMPTY rather than merely present, and whose `aria-busy` is `false` at rest · **Copy all named for its content**, with the label asserted LONGER than a bare "Copy" and `.k-stream-copy` asserted absent (§7.1 condition 1) · the command reference as `aria-expanded` + `aria-controls`, opening, closing, and returning focus to its toggle · **a real command run end to end through the second factor**, with a live TOTP, the pending command retried, and the output read back out of xterm's own accessibility layer · the revalidation dialog's `role`/`aria-modal`/`aria-labelledby`, focus into the field, its visible label, and `Esc` out · the no-second-factor refusal with `role="alert"`, its link, the console absent, and the two shipped hex literals asserted gone · 1.4.10 at 320 CSS px · axe WCAG 2.2 AA over the WHOLE page in both themes (L-037), with the canvas excluded as the deferred INTERIOR and the gaps applied ONE AT A TIME. PHP tier **360 / 1679**, 0 skips, unchanged | **red observed:** `installer/admin/terminal.php renders .k-error with the hidden attribute, but that class declares display: flex and has no .k-error[hidden] { display: none } rule — the element stays PAINTED while the DOM says it is hidden` — `keel-verify`'s own check, FAILING before the fix. **Fourth occurrence of that defect class and the second running that a check caught it instead of a person**; it had never been reachable, because this dialog is the first consumer in the tree to ship a `.k-error` hidden. **NEW-33 is closed with a reproduction, not an assertion:** the spec fails on any of five shipped Spanish literals (`autenticacion`, `Referencia rapida`, `Sesion de terminal expirada`, `Comandos`, `Terminal integrado`) surviving in the rendered body, AND on any `terminal.`-shaped string in the text — L-046's other half, since a MISSING key renders as its own key, with the lookbehind L-046 itself corrected for (the dev bar prints `terminal.php`). **135 keys × 20 catalogues**, spliced as TEXT with every file re-parsed to prove no other root moved; parity check reports 140 files across 7 sets. **A defect fell out of the i18n rather than the design:** `help` carried six hardcoded category labels and the screen its own copy of nine, so `backup`, `update` and `config` printed their raw slug. **A finding recorded and deliberately NOT fixed:** the first command of every terminal session demands a fresh second factor (`checkRevalidation()` reads a session key that is unset, so `time() - 0 > 600`); seeding it at page load would weaken an owner-only control to shorten one test, so the test drives the whole path — which is also the only thing that exercises the dialog's SUCCESS branch. **One defect of my own, moved rather than relaxed:** the code field's label was asserted visible before the dialog opened, where a label inside a hidden dialog is correctly hidden. `keel-verify` **23 checks: 17 pass, 6 warnings**, unchanged; lint on the three touched PHP files **0 errors** | `tests/E2E/artifacts/test-results/terminal-*-chromium/` — in particular `terminal-*a-real-command-still-runs*/trace.zip` (the shell proven to execute after 500 lines of JS moved out of the PHP file) and `terminal-*every-string-on-the-screen*/trace.zip` (NEW-33) |
| **6 of 6 — the specialised screens, SLICE 3: entry 12 (AI chat)** (2026-08-11, D-108) | observed | `tests/E2E/ai-chat.spec.js` — **25 tests**, all passing on `KPORT=8183` (bind confirmed, owning PID `php83 93286` checked per L-021, no `Server:` header per L-011, reseeded `--reset` per L-028), plus `tests/Integration/AiChatValidateKeyHttpTest.php` (2 tests) in the PHP tier. The screen is driven in BOTH of its mutually exclusive server-side states: with no provider configured (the seed's own state) and with one, through a `tests/E2E/fixtures/reset-ai-provider.php` fixture that is reversible and idempotent. `api/ai-chat.php` is intercepted at the browser so the whole-response shape `chat-engine.php` really produces is rendered without a network call. Driven: the shell restored and exactly ONE `<h1>` · the transcript's `role=log`/`aria-live=polite`/`aria-relevant=additions` and the absence of any `aria-live=assertive` · the three starters and the empty-state sentence · the context row · the composer's real `<label>`, its `aria-describedby` hint, Enter sending and Shift+Enter adding a line · each turn as a focusable `<article>` named "You, 14:02" / "Klytos AI, 14:02" · the always-present turn actions reached by FOCUS, not hover · a done and a failed tool call · the unreachable-model `role=alert` with the composer still enabled · the send button `aria-busy` with the response held open so the busy state is observed rather than inferred · the history disclosure's `aria-expanded`, Esc and focus return · the four `?panel=` URLs redirecting · 320 CSS px reflow · the 760px transcript · whole-page axe at `wcag2a/2aa/21a/21aa/22aa` in BOTH themes, scanned WITH a turn and the history panel open (L-037) | **Red first: observed** on the bug-fix reproduction — `red observed: A key that was never issued was reported valid — the endpoint tested nothing. Failed asserting that true is false.` at `AiChatValidateKeyHttpTest.php:64`, against the shipped `strlen($apiKey) > 10`. **FOUR REAL DEFECTS, and every one was found by LOOKING at the rendered screen after 21 assertions had passed in silence** — which is the finding of this slice: an unsized `<svg>` rendering at the SVG default of 300 × 150, so a one-line user bubble measured 196px tall; `.k-field`'s `flex-direction: column` inherited by the composer field, putting the 28px send button UNDER the composer because `display: flex` says nothing about direction (build rule 1's EIGHTH mechanism, and the first where the losing rule was one I never set); `textarea.k-control`'s `min-height: 88px` at (0,1,1) beating `.k-conv-input`, so §1's "auto-grows 34 → 120px" opened at 88; and `.k-status-line` painting a tinted strip when EMPTY, on this screen and on Logs and Terminal too — fixed in the shared layer. Each proven by planting the defect back and watching its own test go red, then restoring byte-identically. **A fifth defect of my own making, caught the same way:** the success announcement was a DOUBLE announcement — the transcript is already a polite log that reads the turn out — so the region is now cleared on success and `ai_chat.sent` was removed from all 20 catalogues rather than left an orphan. **Three shipped defects close here:** two `<h1>`s (and ZERO on a `?panel=` URL); the no-provider composer that PHP hid and JavaScript un-hid and focused, now structurally absent; and `validate_key`. **An existing test was CORRECTED, not relaxed:** `AdminGateHttpTest`'s NEW-31 assertion now follows the redirect and asserts the refusal at the destination, which proves the escalation closed rather than moved. **34 keys × 20 catalogues**, spliced as TEXT with every file re-parsed. Whole browser tier **513 passing in 9.3 min**, 0 failures; PHP **362 / 1692**, 0 skips; `keel-verify` **23 checks: 17 pass, 6 warnings**, unchanged; lint on the touched PHP files identical to HEAD | `tests/E2E/artifacts/test-results/ai-chat-*-chromium/` — in particular `ai-chat-*BESIDE-the-composer*/trace.zip` and `ai-chat-*nothing-to-say*/trace.zip` (the two defects a stylesheet reading would never have surfaced) |
| **7 of 7 — the unblocked screens, SLICE 1: entry 44 (Dashboard)** (2026-08-11, D-110) | observed | `tests/Unit/UpdaterCachedStateTest.php` — **7 tests**, and `tests/Integration/DashboardHttpTest.php` — **10 tests**, all passing; PHP tier **362 / 1692 → 379 / 1744**, 0 skips. The FIRST consumer of the `overview-stats` template, so the run carries the template's own contract as well as the screen's: the five stat cards asserted BY COUNT so the old screen's version cards cannot return, each card's `aria-labelledby` binding value to label, every linked card's destination read out of its `href`, the *Failing checks* card asserted to be a `<div>` and `health.php` asserted ABSENT from the page, the setup panel asserted to be a real `<ol>` whose every step names its state in WORDS, *Build now* asserted to be a CSRF-carrying form and never a link, the indexing banner asserted `role="status"` and asserted to contain no `<form>`, no `<button>` and no checkbox, and the whole screen asserted to arrive server-rendered. **The browser tier is OWED and is NOT claimed** — no axe, no geometry, no both-themes pass, no 320 px reflow, no capture-and-look. | **red observed:** `Error: Call to undefined method Klytos\Core\Updater::getCachedUpdateState()` — `ERRORS! Tests: 7, Assertions: 0, Errors: 7`, seen before the method existed. It names the absent behaviour and not a broken import. **THE SURVEY FINDING:** the *Pending updates* figure had exactly one source, `checkForUpdate()`, which BLOCKS on a GitHub request whenever the six-hour cache is cold — on the screen a person lands on after every login. The fix returns **three** states, because `getCachedRelease()` answers `null` both for *up to date* and for *nobody has checked*, and on a stat card those are opposite facts. **ONE REAL DEFECT, and no reading of the file would have found it:** the stat renderer was a closure defined ABOVE `templates/sidebar.php`, so `use ( $spriteUrl )` captured `null` — a closure binds at DEFINITION time — and every card fataled on `klytos_admin_icon()`'s `string` parameter under `strict_types`. The whole page was a 500. It was caught by `SecurityHeadersHttpTest::testTheCspNonceMatchesTheNonceInTheMarkup()`, written three sprints earlier for another purpose, which asserts the admin emits a nonced script and received a page with no scripts at all. | `tests/Integration/DashboardHttpTest.php`, `tests/Unit/UpdaterCachedStateTest.php`, commit `e26ac2f` |
| **7 of 7 — the unblocked screens, SLICE 2: entry 13 (Tasks)** (2026-08-11, D-111) | n/a — out of scope | `tests/Integration/TasksHttpTest.php` — **11 tests, 60 assertions, ZERO skips**, with `tests/E2E/fixtures/reset-tasks.php` building its own population **through the real `TaskManager`** (a check over a zero population is not evidence — D-079). PHP tier **379 / 1744 → 390 / 1804**, 0 skips. Driven per STATE: the three stat cards asserted **by count** so a fourth cannot be added back from the manifest alone, each bound to its label by `aria-labelledby`; the filters asserted to be LINKS carrying `aria-current` with `role="tab"` asserted ABSENT; an unknown `?status=` resolving to the first rather than failing; the two bodies asserted mutually exclusive with `xor`; the filtered empty state and the **good-news** empty state asserted to be DIFFERENT sentences, the second **REACHED** by emptying the queue through the fixture and putting it back rather than skipped past; a refused CSRF post asserted to REPORT itself; every row action asserted to carry an `aria-label` naming its row; and no catalogue key on the page — checked against the catalogue's own **key list**, because `tasks.php` and `tasks.manage` are both legitimately present and neither is copy. **The browser tier is OWED and NOT claimed** — no axe, no geometry, no both-themes pass. | **`n/a — out of scope`, the honest value rather than `observed` borrowed from a neighbour:** this slice added no pure function of its inputs — it is markup over `TaskManager` methods that already existed and already have their tests. **THE SURVEY FINDING, the product's and not the delivery's:** a Klytos task has **no due date anywhere** — ten fields on `create()`, none of them one, and nothing in `update()`'s allow-list, in storage, in the MCP tools or in the review widget — so §13's *Due this week* and *Overdue* would be numbers nobody measured (DR-013). **AND THE DEFECT THIS SLICE FOUND IN THE LAST ONE:** nothing sized the glyph inside `.k-stat-tile`, and `klytos_admin_icon()` writes no `width`/`height`, so **entry 44's five stat glyphs each rendered at the SVG default of 300 × 150** inside a 32px tile — **L-048's NINTH mechanism**, shipped in `e26ac2f` and invisible to a tier that asserts markup and never geometry. **Four shipped defects closed:** not one `__()` call in the file; a refused CSRF post reporting nothing (the FIFTH screen); the manager's raw English exception in twenty locales; row actions drawn as `✓`/`✕` with only a `title`. **And `keel-verify` caught the fixture's own seed text** before it was committed — a task description quoting filler copy is indistinguishable from filler copy, and the check is right not to try. | `tests/Integration/TasksHttpTest.php`, `tests/E2E/fixtures/reset-tasks.php`, commit `1bfa568` |
| **7 of 7 — the unblocked screens, SLICE 3: entry 18 (Agent payments)** (2026-08-11, D-112) | n/a — out of scope | `tests/Integration/X402DashboardHttpTest.php` — **11 tests, 132 assertions, ZERO skips**, with `tests/E2E/fixtures/reset-x402.php` seeding a **deterministic** 30-day series (seven transactions across three days, unambiguous peak) — a random one makes the chart's headline unassertable, and the headline IS the accessible answer. PHP tier **390 / 1804 → 401 / 1936**, 0 skips. **The first evidence anywhere in this build for `template-overview-stats.md` §4's CHART PATTERN**, so the run carries that contract as well as the screen's: the `<svg>` asserted `role="img"` with an `aria-label` carrying the total and the peak; the `<svg>` asserted to carry its own `width` and `height` ATTRIBUTES (L-048); a real `<table>` asserted to FOLLOW the chart **by DOM position** with all **thirty** rows, not a summary; and the `<details>` asserted `open` (adaptation 92). Plus: four stat cards asserted **by count** so a fifth cannot be built later from the manifest alone; the counts **read back off the page** rather than re-derived from the fixture (L-035); money asserted to carry its currency as TEXT; both detail cards asserted to be real captioned tables; the empty state **REACHED** by removing the population and putting it back, with *Avg. price* asserted `—` and NOT `0` and no chart drawn at all; and no catalogue key on the page, checked against the catalogue's own key list. **The browser tier is OWED and NOT claimed** — no axe, no geometry, no both-themes pass. | **`n/a — out of scope`, the honest value rather than `observed` borrowed from a neighbour:** this slice added no pure function of its inputs — it is markup over `X402\Stats` methods that already existed. **THE SURVEY FINDING, the product's for the third slice running:** §18's *Settlement lag* cannot be computed — a transaction records exactly ONE instant, `created_at`, with `facilitator_ok` a bare bool and `tx_hash` a frequently-empty string beside it, so the lag is between one point and nothing (**DR-014**). **TWO defects of my own, both caught by the tier and neither visible in source:** the fixture used the collection name `x402_transactions` where the constant is `x402-transactions`, so the screen was reading an empty log and seven assertions failed at once; and the provider card called `getName()` on an interface that declares `getLabel()`. | `tests/Integration/X402DashboardHttpTest.php`, `tests/E2E/fixtures/reset-x402.php`, commit `d71a704` |
| **7 of 7 — THE BROWSER TIER for entries 44, 13 and 18** (2026-08-29, D-113) | observed | `tests/E2E/overview-stats.spec.js` — **32 passing, 4 skipped**, on playground `KPORT=8201` (bind confirmed, owning PID `php83 99500` checked per L-021, **no `Server:` header** per L-011, reseeded `--reset` per L-028; `keel-doctor --check` green, 14 rows, before the run per D-067). One file for three screens because what is under test is mostly the TEMPLATE. Driven per screen and per THEME: the stat tile asserted **32px with an 18 × 18 glyph** — the assertion this file exists for, because 300 × 150 is what an unsized `<svg>` renders at and that is exactly what shipped on entry 44 for a whole commit (L-048); the linked stat card asserted to be ONE anchor taking the focus ring around the whole card; **axe-core at `wcag2a/2aa/21a/21aa/22aa` on the WHOLE PAGE in both themes**, 6 runs; WCAG 1.4.10 asserted by TRYING to scroll at 320 CSS px rather than by reading a width (D-079); the chart's real bars with its `<details>` table beside it and REPLACED by it below 900; and **twelve full-page captures** (3 screens × 2 themes × 1440/320), every one of them READ. The 4 skips are declared conditionals with their reason on the skip, not silent passes: Tasks and Agent payments link nowhere from their stat rows, and Tasks renders no banner. | **TWO REAL DEFECTS, and the more expensive one was found by LOOKING, not by asserting.** (1) **Entry 18's three tables had no `grid-template-columns` and fell to `klytos-components.css`'s deliberate `1fr` fallback — ONE CELL PER ROW at every width**, shipped in `d71a704`, turning a 1000px viewport into a **5731px** page. Nothing asserted it and nothing could have at the level the tests were written: the table WAS present, WAS open and DID follow the chart in the DOM, all true of a one-column table. **It was the capture that saw it** — L-048's duty on its tenth mechanism, and the first time the image rather than an assertion is the only witness. Fixed as adaptation 95 and now pinned by a track-count assertion. (2) **A link inside a tinted banner was painted `--color-acento` over `--tinte-aviso` = 3.68:1 in light** on entries 44 and 18 — the build's own over-broad link enumeration, against a rule the delivery itself states (text on a tint is `--sobre-tinte-*`). Fixed as adaptation 96. **red observed:** `5 failed` on the planted defects, with `expect(tracks).toBe(3)` receiving `1` and axe reporting `color-contrast: 4 node(s)` where the pre-fix probe measured `a[data-testid="dashboard.indexing_link"]` at `insufficient color contrast of 3.68 (foreground color: #0e8074, background color: #e7e0d7)`. Both fixes were proven by planting the defect back and watching the test go red — `5 failed`, naming `x402: the data table has real COLUMNS at 1440, not one cell per row` (`expect(tracks).toBe(3)` received `1`) and all four `a link inside a tinted banner takes the tint's colour, not the accent` rows — then restored and diffed byte-identical. The prior red is on record too: before the banner fix, axe measured `color-contrast` on `a[data-testid="dashboard.indexing_link"]` at **3.68** against an expected 4.5.** **And a TEST defect separated from the product ones, which was part of the work:** the first run reported four violations belonging to nobody's screen — `.devbar-tab-content` scrollable with no focusable content, and three `.devbar-*` contrast nodes — because this spec omitted the `DEV_ONLY_SURFACES` exclusion that the other fourteen whole-page scans in the tier all carry. `fixtures.js` already records that exact set. | `tests/E2E/overview-stats.spec.js`, `tests/E2E/artifacts/captures/*.png` (12), `tests/E2E/artifacts/test-results/overview-stats-*-chromium/trace.zip` |
| **7 of 7 — the unblocked screens, SLICE 4: entry 7 (Analytics)** (2026-08-29, D-114) | observed | `tests/Unit/AnalyticsDenseSeriesTest.php` — **7 tests**; `tests/Integration/AnalyticsHttpTest.php` — **12 tests, 98 assertions, ZERO skips** over the deterministic population in `tests/E2E/fixtures/reset-analytics.php` (12 pageviews across four days of a 30-day window, one unambiguous peak); and the screen JOINS the template's shared browser tier, `tests/E2E/overview-stats.spec.js`, now **43 passing** on `KPORT=8202` (bind, owning PID `php83 38774`, no `Server:` header, `--reset` reseed, `keel-doctor --check` green first). PHP tier **401 / 1936 → 420 / 2043**, 0 skips. Driven per state and per theme: four stat cards asserted **by count** so a fifth cannot be built later from the manifest alone, with `time`/`bounce`/`agent` asserted ABSENT by name; the figures **read back off the page** (L-035); the visitor-day note asserted present on a 30d range and ABSENT on 24h; the period chips asserted to be links with `aria-current` and `role="tab"` asserted absent; an unknown `?period=` asserted to resolve to the default; the chart asserted to be a **`<polyline>` and to carry NO `<rect>`**, so the second consumer cannot become a second pattern; the series asserted to have **30 vertices for 30 days**, not 4 for the four days with traffic; the table's real grid tracks at 1440 (D-113's assertion, now written for every consumer); the below-900 replacement; the empty state **REACHED** by removing the population and putting it back, with *Avg. views/day* asserted `—` and not `0`; whole-page axe at WCAG 2.2 AA in **both themes**; and four full-page captures READ. | **red observed:** `Error: Call to undefined method Klytos\Core\AnalyticsManager::denseDailyViews()` — `ERRORS! Tests: 7, Assertions: 0, Errors: 7`, seen before the method existed; it names the absent behaviour and not a broken import. **THE SURVEY FINDING, the product's for the FOURTH slice running:** three of §7's five stats are unmeasurable from what Klytos stores, and the reason is the analytics engine's privacy design rather than a gap — one instant per pageview, no session, no user agent (**DR-015**). **AND A SHIPPED DEFECT FOUND BY THE FIXTURE, ESCALATED AND DELIBERATELY NOT FIXED HERE:** `AnalyticsManager::prune()` reads `$entry['id']`, which `recordPageView()` never writes, so `delete( 'analytics', '' )` throws `InvalidArgumentException: Invalid record ID: ''`. The daily `klytos.analytics_prune` cron calls it with 90 days — so on any install holding data past the retention window the job **fatals, and the 90-day retention has never once run**. The root is deeper than the missing field: `StorageInterface::list()` returns `$records[]` with the ids discarded, so nothing that reads it can delete a specific record. Escalated per Keel's 'looks like a security problem' rule rather than widened into a screen slice. **Two test defects of my own, separated from the product's:** an expectation of 5 visitors where the product correctly answers 6 — the visitor hash is salted per DAY, so the figure is visitor-days, which is exactly what the card's own note says, and the expectation was corrected rather than the screen; and a meaningless `assertLessThan( $chart, $table + 1, 'both are present' )` that fails whenever the table follows the chart, which is the thing the next line requires. | `tests/Unit/AnalyticsDenseSeriesTest.php`, `tests/Integration/AnalyticsHttpTest.php`, `tests/E2E/overview-stats.spec.js`, `tests/E2E/artifacts/captures/analytics-*.png` (4), `tests/E2E/artifacts/test-results/overview-stats-analytics-*-chromium/trace.zip` |
| **7 of 7 — the unblocked screens, SLICE 5: entry 4 (Assets)** (2026-08-30, D-118 + D-119) | observed | `tests/Unit/AssetQueryTest.php` — **10 tests**; `tests/Integration/AssetsHttpTest.php` — **12 tests, 103 assertions, ZERO skips**; and `tests/E2E/assets.spec.js` — **17 passing** on `KPORT=8204` (bind, owning PID, no `Server:` header, `--reset` reseed, `keel-doctor` green first). PHP tier **460 / 2160 → 472 / 2264**, 0 skips. **The FIRST consumer of `template-gallery-grid.md`**, and the first screen this build re-architected rather than re-skinned: the shipped one rendered its grid, filters and pagination in the BROWSER and showed nothing with scripting off. Driven: the grid asserted to arrive in the HTML with its tiles and `Populated by JS` asserted ABSENT · each kind chip asserted to select a different set BY COUNT · `Unused` excluding exactly the asset in use · the *No alt text* chip on exactly ONE tile and asserted to be an anchor at `#alt`, whose destination is then CLICKED and the field found · delete disabled on exactly one with the reason and count in its `aria-label`, **and the same rule enforced on the POST** · both empty states asserted to be DIFFERENT sentences · a genuinely refused CSRF · preview 96px and glyph 32×32 · the grid asserted to have MORE THAN ONE COLUMN at 1440 · **the hover-revealed actions proven reachable BY KEYBOARD** · a ring on the primary link AND on the action with no nested-interactive tile · whole-page axe in BOTH themes · the 320px reflow by TRYING to scroll · four captures READ. | **red observed:** `Error: Call to undefined method Klytos\Core\AssetManager::query()` — `ERRORS! Tests: 10, Assertions: 0, Errors: 10`, before the method existed. **THE SURVEY FINDING is the architecture, not a missing capability** — for the first time in five slices the product holds everything §4 draws, so no Design Request was needed. **THREE DEFECTS THE CAPTURES FOUND AND EVERY ASSERTION PASSED** (all of them TEXT, the class structure cannot reach): a raw `{size}` placeholder, a usage count with no number, and an `<h1>` reading 'Files' where `navigation.md:62` states it is 'Assets'. **A FOURTH in code just written:** the usage link at `#5B8DEF`, 4.31:1 in dark, because `.k-tile` was not added to the link layer's ENUMERATION — the third time that trap has fired, and its own comment warns about it. **A PLAYGROUND GAP** made every thumbnail 404: the router mapped unknown paths into `installer/public/` while the product writes assets to the WEB ROOT, so the playground disagreed with production; found through the read-back duty before any assertion ran, fixed with `realpath()` containment (traversal → **403**, verified). **Two defects of my own separated from the product's:** a hover-reveal opacity read once mid-transition (D-078's shape on opacity), and — on another screen — `TasksHttpTest`'s refused-CSRF test that **passed for the wrong reason** and led to fixing the harness (L-051). | `tests/Unit/AssetQueryTest.php`, `tests/Integration/AssetsHttpTest.php`, `tests/E2E/assets.spec.js`, `tests/E2E/artifacts/captures/assets-*.png` (4), `tests/E2E/artifacts/test-results/assets-*-chromium/trace.zip` |
| **7 of 7 — the unblocked screens, SLICE 6: entry 21 (Blocks)** (2026-08-30, D-120) | observed | `tests/Unit/BlockUsageCountsTest.php` — **6 tests**; `tests/Integration/BlocksHttpTest.php` — **7 tests, 37 assertions, 0 skips**; `tests/E2E/blocks.spec.js` — **13 passing** on `KPORT=8205`. PHP tier **472 / 2264 → 485 / 2294**. The SECOND consumer of `gallery-grid`, so the layer's own contract is NOT re-asserted — only what differs: the wireframe preview (120px against an asset's 96px), asserted `aria-hidden` with the link still named by text; three groups each with its OWN labelled `<ul>`, every `aria-labelledby` asserted to point at a heading THAT EXISTS; the category labels asserted to come from the catalogue; the usage counts asserted at 2 / 1 / none — three DIFFERENT answers, so a constant cannot pass; the group rhythm measured as a real gap between bounding boxes; whole-page axe in BOTH themes; the 320px reflow; no placeholder on the page; four captures READ. | **red observed:** `Error: Call to undefined method Klytos\Core\PageTemplateManager::blockUsageCounts()` — `ERRORS! Tests: 6, Assertions: 0, Errors: 6`. **THE DEFECT THIS SLICE IS ABOUT:** `blockUsageCounts()` read a plain `blocks` list of ids where `addBlock()` writes `structure` as `['block_id' => …, 'order' => …]`, so every tile on the real screen read "In no template" — **and its unit test passed, because the test's own fixture seeded the shape the method expected.** A fixture that invents the shape it tests against proves only that the code agrees with the fixture. The DRIVEN screen caught it, and the test now seeds through `save()` and `addBlock()` so the shape cannot drift from what the product writes. L-051's family, from the other direction. **AND THE CAPTURE FOUND WHAT EVERY ASSERTION PASSED, for the tenth time:** three category groups with no vertical rhythm, each `<h2>` flush against the tile above it — correct markup, correct labelling, and it looked broken. **Two shipped defects fixed on the way:** hard-coded English category labels on a twenty-catalogue product, and a failed preview showing the manager's English exception. **The 8 skips in the full-suite run are the `DatabaseStorage` tier**, skipping because the Docker daemon was not running — the designed behaviour verified in D-117, not a regression. | `tests/Unit/BlockUsageCountsTest.php`, `tests/Integration/BlocksHttpTest.php`, `tests/E2E/blocks.spec.js`, `tests/E2E/artifacts/captures/blocks-*.png` (4), `tests/E2E/artifacts/test-results/blocks-*-chromium/trace.zip` |
